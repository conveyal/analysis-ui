...
function mapStateToProps ({
  ...
  const defaultFeedId = scenario.feeds.length > 0
    ? scenario.feeds[0].id
    : ''
  const modificationsByType = scenario.modifications.reduce((modifications, modification) => {
...
class Scenario extends Component {
  ...
  render () {
    const {bounds, bundleName, candidateScenarios, children, id, modificationsByType, name, projectId, push} = this.props
