...
export class Number extends PureComponent {
  ...
  _onChange = (e: Event & {target: HTMLInputElement}) => {
  ...
  _renderHelpBlock() {
    ...
    if (isNumber(max)) {
      helpText = `Enter a number between ${helpText}-${max}`
    } else {
      helpText = `Enter a number greater than ${helpText}`
    }
    return (
