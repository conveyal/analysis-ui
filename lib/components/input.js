...
class StateToValue extends PureComponent {
  ...
  static getDerivedStateFromProps(props, state) {
    if (props.value === null) return {value: undefined}
...
export function Input(p) {
  const {label, placeholder, units, ...props} = p
  ...
  if (units) {
    return (
      <div className='InputWithUnits'>
        <input
          ...
          placeholder={placeholder || units || label || p.name}
          id={propsToId(p)}
          {...p}
          onChange={onChange}
          value={value}
    ...
    return (
      <input
        className='form-control'
        placeholder={placeholder || label || p.name}
        id={propsToId(p)}
        {...p}
        onChange={onChange}
        value={value}
...
export function Number(p) {
  ...
  const id = propsToId(p)
  const {className, onChange, ...propsLessOnChange} = p
  const {max, min = 0} = p
  const groupProps = {...p}
  if (hasError) {
