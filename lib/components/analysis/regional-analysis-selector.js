 ...
 * on the map
 */
export default class RegionalAnalysisSelector extends PureComponent {
  static propTypes = {
  ...
  componentWillReceiveProps (nextProps) {
    ...
    if (regionalAnalyses !== this.props.regionalAnalyses) {
      this.setState({
        sortedRegionalAnalyses: sortByName(regionalAnalyses || [])
      })
    }
  }
...
function Analysis ({analysis, onSelect, deleteRegionalAnalysis}) {
  const complete =
    ...
    <li className='list-group-item' key={`analysis-selector-${analysis.id}`}>
      ...
      {!complete &&
        <span>
          ...
          <small>
            {analysis.status.complete} of {analysis.status.total} complete
          </small>
        </span>}
