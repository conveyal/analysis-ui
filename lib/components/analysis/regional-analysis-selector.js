...
export default class RegionalAnalysisSelector extends Component {
  ...
  _deleteRegionalAnalysis = memoize((analysisId: string) => () =>
    window.confirm('Are you sure you wish to cancel this regional analysis?') && this.props.deleteAnalysis(analysisId))
...
function Analysis ({analysis, onDelete, onSelect}) {
  ...
  return complete
    ...
    ><span className='list-group-item-heading'><Icon type='arrow-right' /> {analysis.name}</span></a>
  : <li className='list-group-item'>
    <span className='list-group-item-heading'>
      <span><Icon type='cloud' /> {analysis.name} - {analysis.status.complete} of {analysis.status.total} complete</span>
      <a
