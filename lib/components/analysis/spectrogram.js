...
export default class Spectrogram extends Component {
  ...
  draw = () => {
    ...
    const scaleX = width / MINUTES
    const scaleY = height / bins
    ...
    for (let minute = 1; minute < MINUTES; minute++) {
      for (let iteration = 0; iteration < iterations; iteration++) {
        const val = cumulative[iteration][minute - 1] + data[iteration][minute]
    ...
    for (let iteration = 0; iteration < iterations; iteration++) {
      for (let minute = 0; minute < MINUTES; minute++) {
        let yPixel
        const val = cumulative[iteration][minute]
        if (difference) {
          if (val >= 0) {
    ...
    const id = ctx.createImageData(MINUTES * scaleX, bins * scaleY)

    for (let gridx = 0; gridx < MINUTES; gridx++) {
      for (let gridy = 0; gridy < bins; gridy++) {
        const cell = (gridy * MINUTES) + gridx
        ...
        for (let xoff = 0; xoff < scaleX; xoff++) {
          for (let yoff = 0; yoff < scaleY; yoff++) {
            const pixel = (((gridy * scaleY) + yoff) * MINUTES * scaleX) + ((gridx * scaleX) + xoff)
    ...
    const x = isochroneCutoff * scaleX
    ...
    // draw scales on top
    // x scale
    const offset = (bins * scaleY) - (TEXT_HEIGHT / 1.75)
    ...
    if (difference) {
      ...
      yTicks.slice(1).forEach((tick, i, arr) => {
        const yoff = (bins - 1 - yScale(tick)) * scaleY
    ...
    // color scale
    const colorOffsetX = scaleX * (MINUTES - 5)
    const textOffsetX = colorOffsetX - 45
    let offsetY = (bins * scaleY) - 150

    ctx.strokeStyle = '#fff'

    for (const p of [...positiveColorStops, 1].reverse()) {
