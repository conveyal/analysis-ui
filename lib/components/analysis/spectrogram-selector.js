...
export default class SpectrogramSelector extends Component {
  ...
  renderSpectrogram () {
    ...
    const scenarioColor = isFetchingIsochrone
      ? process.env.STALE_SPECTROGRAM_COLOR
      : process.env.SCENARIO_SPECTROGRAM_COLOR
    const comparisonColor = isFetchingIsochrone
      ? process.env.STALE_SPECTROGRAM_COLOR
      : process.env.COMPARISON_SPECTROGRAM_COLOR
...
function computeSpectrogramDifference (minuend, subtrahend) {
  ...
  for (let i = 0; i < outSize; i++) {
    ...
    const minuendComponent = minuend[Math.random() * minuend.length | 0]
    const subtrahendComponent = subtrahend[Math.random() * subtrahend.length | 0]
