...
export default class ModeSelector extends PureComponent<void, Props, void> {
  render (): React$Element<*> {
    ...
    return <Group label='Modes'>
      <br />
      <div className='btn-group' role='group' aria-label={messages.analysis.mode}>
        ...
        <label htmlFor='mode-car-park' className={`btn btn-default ${nonTransitMode === CAR_PARK ? 'active' : ''}`}>
          <input
            type='radio'
            className='sr-only'
            name='streetMode'
  ...
  selectNonTransitMode = (e: Event & {target: HTMLInputElement}): void => {
    ...
    setProfileRequest({
      ...
      transitModes: e.target.value === CAR_PARK ? 'TRANSIT' : profileRequest.transitModes
    })
  }
