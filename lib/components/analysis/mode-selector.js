...
const ALL = 'ALL'
const ALL_TRANSIT = [BUS, RAIL, TRAM, SUBWAY, FERRY, CABLE_CAR, GONDOLA, FUNICULAR].join(',')
...
export default class ModeSelector extends Component {
  ...
  _hasAllTransit () {
    return ALL_TRANSIT.split(',').map(mode => this.props.transitModes.indexOf(mode) !== -1)
  .every(Boolean)
  }
  ...
  _toggleTransitMode = memoize(mode => () => {
    var transitModes
    if (mode === ALL) {
      transitModes = this._hasAllTransit() ? '' : ALL_TRANSIT
    } else {
    ...
    const accessModes = transitModes === '' && this.props.accessModes === CAR_PARK
      ? WALK
