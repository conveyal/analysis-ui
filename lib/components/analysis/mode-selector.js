...
export default class ModeSelector extends PureComponent {
  ...
  _toggleTransitMode = memoize(mode => () => {
    const {profileRequest, setProfileRequest} = this.props
    let transitModes = this._hasTransit(mode)
      ? without(profileRequest.transitModes.split(','), mode).join(',')
      : `${profileRequest.transitModes},${mode}`

    if (transitModes === '') {
      window.alert('This version of Conveyal Analysis requires at least one transit mode turned on.')
      transitModes = mode
    }
