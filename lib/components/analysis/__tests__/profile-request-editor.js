...
describe('Components > Analysis > Profile Request Editor', () => {
  ...
  it('should react to input correctly', () => {
    ...
    wrapper.find({name: 'fromTime'}).dive()
      .find('Text').dive().find('Input').dive()
      .find('input').simulate('change', ch('01:00'))
    wrapper.find({name: 'toTime'}).dive()
      .find('Text').dive().find('Input').dive()
      .find('input').simulate('change', ch('13:00'))
