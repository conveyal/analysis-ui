...
export default class SinglePointAnalysis extends Pure {
  ...
  setComparisonVariant = (e: ReactSelectResult) => {
    ...
    const scenario = scenarios.find(s => s._id === comparisonScenarioId)
    if (e) {
  ...
  renderComparisonSelect () {
    ...
    const chosenScenario = scenarios.find(s => s._id === comparisonScenarioId)
...
function webMercatorBoundsToGeographic ({
  ...
  const nw = LeafletMap.prototype.unregion([west + 1, north], zoom)
  const se = LeafletMap.prototype.unregion(
