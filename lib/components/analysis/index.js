...
export default class SinglePointAnalysis extends Component {
  ...
  fetchIsochrone = ({ chosenIndicator, chosenCutoff }) => {
    const { projectId, scenarioId, bundleId, modifications, isochroneCutoff, fetchIsochrone, isochroneLatLng, currentIndicator, workerVersion, variantIndex } = this.props
    fetchIsochrone({
  ...
  toggleComparison = (e) => this.setState({ ...this.state, compare: !this.state.compare })
  ...
  render () {
    ...
    return <div className='Analysis'>
      ...
      {this.state.compare && this.renderComparisonSelect()}
