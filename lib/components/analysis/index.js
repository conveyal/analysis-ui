...
import {Group, Text, Checkbox, Slider} from '../input'
...
export default class SinglePointAnalysis extends Component {
  ...
  componentDidUpdate () {
    const {spectrogramData, destinationGrid, isShowingIsochrone, isShowingOpportunities, addIsochroneLayerToMap, addOpportunityLayerToMap} = this.props
    if (this.visible) { // make sure that the component hasn't been unmounted
      if (!isShowingIsochrone && spectrogramData) addIsochroneLayerToMap()
