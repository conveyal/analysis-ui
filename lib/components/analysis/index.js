...
export default class SinglePointAnalysis extends Component {
  ...
  render () {
    const { currentIndicator, indicators, doingComparison, comparisonSpectrogramData, comparisonScenarioId, comparisonVariant, scenarios, accessibility, isochroneCutoff, spectrogramData, variantName, projectId, scenarioName, isFetchingIsochrone, uploadGrid, scenarioId, variantIndex } = this.props
    ...
    if (comparisonSpectrogramData) {
      let comparisonScenario = scenarios.find(s => s.id === comparisonScenarioId)
      comparisonScenarioName = `${comparisonScenario.name}: ${comparisonScenario.variants[comparisonVariant]}`
    }
