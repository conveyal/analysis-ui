...
export default class StackedPercentile extends DeepEqualComponent<
  void,
  Props,
  void
> {
    ...
    return (
      <svg style={{width, height}}>
        <g>
          {
            <g
              transform={`translate(${comparisonPercentileCurves
                ? boxPlotWidth
                : 0})`}
            >
              {this.renderBoxPlot({
  ...
  renderYAxis () {
    ...
    const toRender = yTicks.map((tick, i, arr) => {
      ...
      // the line.
      const tickText =
        i === arr.length - 1 ? `${valueText} ${indicator}` : valueText
