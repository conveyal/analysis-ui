...
import type {AggregateAccessibility as AggregateAccessibilityType} from '../../types'
...
type State = {
  percentileOfAccessibility: string
}

export default class AggregateAccessibility extends DeepEqualComponent<State, Props, State> {
  state = {
    /**
  ...
  renderPercentileReadout () {
    ...
    return <div>
      <AggregateAccessibilityReadout
        ...
        accessibilityForPercentile={aggregateAccessibility.percentiles[percentileOfAccessibility]}
        weightedAverage={aggregateAccessibility.weightedAverage} />

      {comparisonAggregateAccessibility && <AggregateAccessibilityReadout
...
const AggregateAccessibilityReadout = pure(({
  name,
  weightByName,
  accessToName,
  percentage,
  ...
  accessToName: string,
  percentage: number,
  accessibilityForPercentile: number,
  ...
  return <div>
    ...
    <br />
    <Icon type='exclamation-triangle' title={messages.analysis.weightedAverageAccessibilityWarning} />
    {sprintf(messages.analysis.weightedAverageAccessibility, fmt(Math.round(weightedAverage)))}
  </div>
})
