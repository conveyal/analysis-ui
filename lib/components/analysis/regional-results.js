...
const NEW_AA = 'new-aggregation-area'
...
export default class RegionalResults extends Component {
  ...
  selectAggregationArea = e => {
    ...
    if (e) {
      if (e._id === NEW_AA) {
        ...
        setAggregationArea({regionId, aggregationAreaId: e._id})
      }
    } else {
      setAggregationArea()
    }
  ...
  renderAggregation() {
    ...
    const aggregationAreaOptions = [
      ...aggregationAreas,
      {
        name: message('analysis.newAggregationArea'),
        _id: NEW_AA
      }
    ]
      ...
      <div>
        <Group label={message('analysis.aggregateTo')}>
          <Select
            ...
            value={aggregationAreas.find(aa => aa._id === aggregationAreaId)}
