...
export default class RegionalAnalysis extends React.PureComponent {
  ...
  _downloadProjectGIS = e => {
    ...
    const p = this.props
    p.fetch({
      url: `${API.Regional}/${p.analysis._id}/grid/tiff?redirect=false`,
      next(response) {
        ...
        {isLoaded ? (
          <>
            <ProfileRequestDisplay
              defaultExpanded
              deleteAnalysis={this._deleteAnalysis}
              renameAnalysis={this._renameAnalysis}
              {...p.analysis}
              {...p.analysis.request}
            />
            ...
            <Group justified>
              ...
              <Button onClick={this._deleteAnalysis} size='sm' style='danger'>
                <Icon icon={faTrash} />{' '}
