...
export default class RegionalAnalysis extends Component {
  ...
  renderAnalysis (analysis) {
    ...
    const { activeRegionalAnalysis, comparisonRegionalAnalysis, regionalAnalyses, minimumImprovementProbability, setMinimumImprovementProbability } = this.props
    let shown = activeRegionalAnalysis === analysis.id

    const comparisons = regionalAnalyses.map(({ name, id }) => { return { label: name, value: id } })
    comparisons.sort((a, b) => a.label < b.label ? -1 : a.label > b.label ? 1 : 0)
    ...
    return <li className='list-group-item'>
      ...
      {shown && <div>
        <Select
          ...
          onChange={(e) => this.selectRegionalAnalysis(analysis.id, e.value)}
          />
        </div>}
