...
import type {RegionalAnalysis, Grid, AggregationArea, AggregateAccessibility as AggregateAccessibilityType, ReactSelectOptions, Indicator, ReactSelectOption} from '../../types'
...
type Props = {
  ...
  setAggregationWeights: ({ aggregationWeightsId: string, projectId: string }) => void,
  uploadAggregationArea: (arg: {name: string, files: FileList, projectId: string}) => void,
...
type State = {
  ...
  aggregationAreaFiles: ?FileList,
  aggregationAreaName: ?string
}

export default class RegionalAnalysisDisplay extends Component<State, Props, State> {
  state = {
  ...
  selectComparisonAnalysis = (chosen: ReactSelectOption): void => {
    const {
  ...
  selectAggregationArea = (e: ReactSelectOptions) => {
    const {projectId, setAggregationArea, addAggregationAreaComponentToMap, removeAggregationAreaComponentFromMap} = this.props
  ...
  selectAggregationWeights = (e: ReactSelectOptions) => {
  ...
  uploadAggregationArea = (e: Event): void => {
    ...
    if (!aggregationAreaName || !aggregationAreaFiles) throw new Error('Attempt to upload null aggregation area')

    uploadAggregationArea({ name: aggregationAreaName, files: aggregationAreaFiles, projectId })
    this.setState({ aggregationAreaName: null, aggregationAreaFiles: null, showAggregationAreaUpload: false })
  }

  /** Render controls for weighted average accessibility */
  renderAggregation () {
    const {aggregationAreas, aggregationAreaId, indicators, aggregationWeightsId, aggregateAccessibility, comparisonAggregateAccessibility, comparisonAnalysis, aggregationAreaUploading, analysis} = this.props
    const {showAggregationAreaUpload, aggregationAreaName, aggregationAreaFiles} = this.state

    // this same conditional is in the render function before this function is ever called, but flow
    // doesn't know that
    if (!analysis) return

    const aggregationAreaOptions = aggregationAreas.map(aa => ({ value: aa.id, label: aa.name }))
    ...
    return <div>
      ...
      {showAggregationAreaUpload && <form onSubmit={this.uploadAggregationArea}>
        ...
        <input
          ...
          disabled={aggregationAreaUploading || !aggregationAreaName || !aggregationAreaFiles}
          type='submit'
          value={messages.analysis.uploadAggregationArea}
          />
      </form>}

      {aggregationAreaId && <Group label={messages.analysis.weightBy}>
        <Select
      ...
      {aggregationAreaUploading && <span><Icon type='fa-spinner' spin />&nbsp;{messages.analysis.uploading}</span>}

      {aggregateAccessibility && aggregationWeightsId && <AggregateAccessibility
        ...
        comparisonAccessToName={comparisonAnalysis && this.findIndicatorName(comparisonAnalysis.grid)}
        comparisonRegionalAnalysisName={comparisonAnalysis && comparisonAnalysis.name}
      />}
    </div>
  }
  ...
  downloadComparisonGIS = (e: Event) => {
    ...
    if (this.props.comparisonAnalysis) {
      ...
      throw new Error('Attempt to dowload comparison analysis that has not yet loaded!')
    }
  }
  ...
  downloadProbabilityGIS = (e: Event) => {
    ...
    if (this.props.analysis && this.props.comparisonAnalysis) {
      ...
      throw new Error('Attempt to download probability surface when both analyses have not loaded.')
    }
  }
  ...
  render () {
    ...
    return <div>
      ...
      <Legend
        breaks={breaks}
        min={comparisonAnalysis != null && differenceGrid != null
          ? differenceGrid.min
          : grid.min}
