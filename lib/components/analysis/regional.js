...
export default function Regional(p) {
  ...
  const cutoffInput = useControlledInput(useSelector(selectCutoff), (v) =>
    dispatch(setSearchParameter('cutoff', v))
  )
    ...
    <>
      ...
      <Stack spacing={4}>
        ...
        {Array.isArray(cutoffsMinutes) && (
          <Select onChange={cutoffInput.onChange} value={cutoff}>
        ...
        {Array.isArray(percentiles) && (
          <Select onChange={percentileInput.onChange} value={percentile}>
            {percentiles.map((p) => (
