...
export default class StackedPercentileSelector extends PureComponent {
  ...
  render () {
    ...
    return (
      <div>
        <h5>Accessibility to <strong>{opportunityDatasetName || `[select opportunity dataset]`}</strong> in <strong>{isochroneCutoff}</strong> minutes (travel time percentile: <strong>{nearestPercentile}</strong>th)</h5>
        <input
