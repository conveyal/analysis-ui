  ...
  render () {
    ...
    return (
      <div>
        {!stale &&
          <h5>
            Accessibility to <strong>{opportunityDatasetName ||
              `[select opportunity dataset]`}</strong> in <strong>{isochroneCutoff}</strong> minutes (travel time percentile: <strong>{nearestPercentile}</strong>th)
          </h5>
        }
        {stale &&
          <h5>
            Accessibility will update after fetching results.
          </h5>
        }
        <input
