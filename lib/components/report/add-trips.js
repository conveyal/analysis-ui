...
import type {AddTripPattern, GTFSStop, Timetable} from '../../types'
...
class AddTrips extends Component<void, Props, void> {
    ...
    return (
      <div>
        <MiniMap bounds={bounds}>
          <AddTripPatternLayer bidirectional segments={modification.segments} />
        </MiniMap>
  ...
  renderTimetable ({
    ...
    const {endTime, headwaySecs, timetableId, name, segmentSpeeds, startTime} = timetable
    ...
    if (timetable.phaseAtStop) {
      ...
      // above it.
      out.push(
        <tr aria-hidden style={{height: 0, border: 0}} key={`${timetableId}-empty`} />
      )

      // TODO how to indicate to screen readers that this is associated with the row above?
      out.push(
        <tr key={`${timetableId}-phase`} style={{borderTop: 0}}>
          <td />
          <td colSpan={6}>
            <div>
              <Phase
