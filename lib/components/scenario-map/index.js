...
type Props = {
  ...
  qualifiedStops: Stop[],
  replaceModification: ({bundleId: string, modification: Object}) => void,
  ...
  _selectStops = (selectedStops: string[]) => {
    ...
    if (activeModification) {
      switch (mapState.action) {
        case 'add':
          this.updateModification({stops: [...(new Set([...activeModification.stops, ...selectedStops]))]})
          break
        case 'new':
          this.updateModification({stops: selectedStops})
          break
        case 'remove':
          this.updateModification({stops: activeModification.stops.filter((sid) => selectedStops.indexOf(sid) === -1)})
          break
      }
    }
    setMapState({
  ...
  _selectHops = (hops: string[][]) => {
    ...
    if (activeModification) {
      switch (mapState.action) {
        ...
        case 'remove':
          this.updateModification({
            hops: activeModification.hops.filter((existing) =>
              hops.findIndex((selected) =>
                existing[0] === selected[0] &&
                existing[1] === selected[1]) === -1)
          })
          break
      }
    }
    ...
    switch (mapState.state) {
      ...
      case MAP_STATE_HOP_SELECTION:
        return <HopSelectPolygon
          hopStops={getHopStopsForModification({feed: activeModificationFeed, modification: activeModification})}
...
function getHopStopsForModification ({feed, modification}: {feed: any, modification: any}) {
  ...
  if (modification.trips !== null) {
    patterns = patterns.filter((pat) =>
      pat.trips.findIndex((t) =>
        modification.trips.indexOf(t.trip_id) > -1) > -1)
  }
  ...
  return candidateHops.map((hop) => [feed.stopsById[hop[0]], feed.stopsById[hop[1]]])
}
