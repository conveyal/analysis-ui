...
export default class TransitEditor extends LayerGroup {
    ...
    if (allowExtend) {
      runAsync(async () => {
        ...
        if (p.modification.type === ADD_TRIP_PATTERN) {
          const tts = p.modification.timetables
          if (tts && tts.length > 0) {
            updateModification({
              segments,
              timetables: tts.map(tt => ({
                ...tt,
          ...
          updateModification({
            segments,
            segmentSpeeds: updateSegmentSpeeds([...p.modification.segmentSpeeds])
          })
        }
      })
    }
  ...
  _deleteStopOrPoint = (index: number) => () => {
    ...
    if (index === 0) {
      ...
      if (p.modification.type === ADD_TRIP_PATTERN) {
        ...
        p.updateModification({
          segments,
          segmentSpeeds: removeFirstSegmentSpeed([...p.modification.segmentSpeeds])
        })
      }
  ...
  async _insertStop (coordinates: LonLatC, index: number) {
    ...
    const insertSpeed = ss => {
      ...
      return this.extendSegmentSpeedsTo(ss, segments.length + 1)
    }
