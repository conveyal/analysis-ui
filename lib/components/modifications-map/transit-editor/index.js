...
export default class TransitEditor extends FeatureGroup {
  ...
  getStopNear (pointClickedOnMap) {
    ...
    if (zoom >= MINIMUM_SNAP_STOP_ZOOM_LEVEL) {
      return getStopNearPoint({
        lonlat: lonlat(pointClickedOnMap),
        radiusPixels: 10,
  ...
  _togglePoint = index => {
    ...
    const currentlyStop = index === 0
      ? segments[0].stopAtStart
      : segments[index - 1].stopAtEnd
  ...
  handleClick = ({latlng}) => {
    ...
    if (allowExtend) {
      let coordinates = lonlat.toCoordinates(latlng)
      let stopId = null
  ...
  handleSegmentDragEnd = e => {
    if (this.draggingSegment >= 0) {
      // make sure we are dragging a segment
      debug(`drag end segment ${this.draggingSegment}`)
      const coordinates = lonlat.toCoordinates(e.latlng)
  ...
  _dragPoint = async (e, {autoCreated, index}) => {
    ...
    const coordinates = lonlat.toCoordinates(pos)
  ...
  _dragAutoCreatedStop ({coordinates, index, snapStop}) {
    let stopId = null
  ...
  async _dragStop ({coordinates, index, snapStop}) {
    const {followRoad, modification, updateModification} = this.props
    const segments = [...modification.segments]
    ...
    if (!isEnd) {
      ...
      const newSegment = await getSegment({
        ...
        from: coordinates,
        segments,
        to: nextSeg.geometry.type === 'LineString'
          ? nextSeg.geometry.coordinates.slice(-1)[0]
          : nextSeg.geometry.coordinates,
        ...nextSeg
      })
  ...
  async insertStop ({coordinates, index, isStop = true, stopId}) {
