...
const coordinatesToLeaflet = memoize(coordinates =>
  ...
  render() {
    ...
    return (
      <>
        ...
        />
        {highlightSegment !== undefined && highlightSegment > -1 && (
          <Polyline
            color={colors.HIGHLIGHT}
            opacity={0.5}
            positions={segments[highlightSegment].geometry.coordinates.map(c =>
              lonlat.toLeaflet(c)
            )}
        ...
        {highlightStop && (
          <Marker
            icon={enlargeIconBy(
              highlightStop.stopId ? newSnappedStopIcon : newStopIcon,
              2
            )}
