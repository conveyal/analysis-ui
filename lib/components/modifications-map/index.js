...
export default class ModificationsMap extends React.Component {
  render() {
    const p = this.props
    const {activeModification, feedsById, mapState} = this.props
    const dim = id => activeModification && activeModification._id !== id
    const modifications = [...p.modifications, activeModification].filter(
      Boolean
    )
  ...
  getHighlightSegmentForModification(modification) {
  ...
  getHighlightStopForModification(modification) {
    ...
    if (p.activeModification) {
      switch (p.mapState.action) {
        case 'add':
          p.updateLocally({
            stops: [
              ...new Set([...p.activeModification.stops, ...selectedStops])
            ]
          })
          break
        case 'new':
  ...
  // TODO Move these into the modification/editor
            ...
            onSelect={fromStop => {
              p.updateLocally({
                ...
                segments: reroute.segmentsFromStop(
                  fromStop,
                  get(p.activeModification, 'segments', []),
                  `${get(p.activeModification, 'feed')}:${fromStop.stop_id}`
                )
              })
            ...
            onSelect={toStop => {
              p.updateLocally({
                ...
                segments: reroute.segmentsToStop(
                  toStop,
                  get(p.activeModification, 'segments', []),
                  `${get(p.activeModification, 'feed')}:${toStop.stop_id}`
                )
              })
              p.setMapState({state: null})
            }}
...
function getHopStopsForModification({feed, modification}) {
  ...
  const hopsForPattern = patterns.map(p =>
    p.stops.map((stop, index, array) =>
      index < array.length - 1 ? [stop.stop_id, array[index + 1].stop_id] : null
    )
  )
