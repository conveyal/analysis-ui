...
export default class CopyTimetable extends Component {
  ...
  async componentDidMount() {
    const p = this.props
    const regions = await p.getTimetables()
    ...
    // modifications.
    let currentRegion = regions.find((region) => region._id === p.regionId)
    ...
    if (currentRegion) {
      currentProject = currentRegion.projects.find(
        (project) => project._id === p.currentProject._id
      )
    ...
    if (currentProject) {
      currentModification = currentProject.modifications.find(
        (modification) => modification._id === p.currentModification._id
      )
    ...
    this.setState({
      ...
      modificationOptions: get(currentProject, 'modifications', []),
      project: currentProject,
      projectOptions: get(currentRegion, 'projects', []),
      region: currentRegion,
      regionOptions: regions,
      timetable: get(currentModification, 'timetables[0]'),
      timetableOptions: get(currentModification, 'timetables', [])
    })
  }
