...
export default class Phase extends PureComponent<void, Props, State> {
  ...
  _setPhaseAtStop = (stop: ReactSelectResult) => {
    this.props.update({phaseAtStop: stop && stop.value})
  }

  _setPhaseFromTimetable = (timetable: ReactSelectResult) => {
    this.props.update({
  ...
  _setPhaseFromStop = (stop: ReactSelectResult) => {
