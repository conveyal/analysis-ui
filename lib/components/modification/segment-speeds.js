...
export default class SegmentSpeeds extends PureComponent<void, Props, State> {
  ...
  _highlightStop = memoize(
    (stop?: GTFSStop) => () => this.props.highlightStop(stop),
    (stop?: GTFSStop) => JSON.stringify(stop) // resolver
  )
  ...
  _setDwellTimeForStop = memoize((index: number) => (seconds) => {
    let {dwellTime, dwellTimes, modificationStops, update} = this.props
    dwellTimes = dwellTimes || modificationStops.map(() => dwellTime)
    ...
    return (
      <div>
        <MinutesSeconds
          ...
          seconds={dwellTime}
        />
        {modificationStops && <Collapsible title='Set individual stop dwell times'>
          {modificationStops.map((stop, index) =>
            <MinutesSeconds
              key={`segment-speeds-stop-dwell-time-${stop.stop_id}`}
              label={stop.stop_name}
              placeholder='Dwell time'
              onBlur={this._highlightStop(null)}
              onChange={this._setDwellTimeForStop(index)}
              onFocus={this._highlightStop(stop)}
              seconds={(dwellTimes || [])[index] || dwellTime}
              />
          )}
        </Collapsible>}
        <InputNumber
