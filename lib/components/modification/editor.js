...
export default class ModificationEditor extends React.Component {
  ...
  _onDescriptionChange = (e: Event & {currentTarget: HTMLTextAreaElement}) => {
  ...
  _customizeModification = (e: Event & {currentTarget: HTMLTextAreaElement}) => {
    try {
      const json = JSON.parse(get(this._refToTextarea, 'value'))
      this._updateLocally(json)
    } catch (e) {
      window.alert('Error editing custom JSON. See console for details.')
      console.error(e)
    }
  }
  ...
  render () {
    ...
    const customizableAttributes = omitAttributes(modification)
    return (
        ...
        <Title>
          ...
          <span className={disableAndDim}>
            <IconLink
              ...
              name={message('modification.deleteModification')}
              type='trash'
            />
            <IconLink
              className='pull-right'
              onClick={copyModification}
              name={message('modification.copyModification')}
              type='copy'
            />
            <IconLink
              className='pull-right'
              onClick={this._showEditModificationName}
              name={message('modification.editModificationName')}
              type='pencil'
            />
          </span>
        </Title>
              ...
              ? <div>
                ...
                <Collapsible title={message('modification.customizeDirectly')}>
                  ...
                  <textarea
                    ...
                    defaultValue={JSON.stringify(customizableAttributes, null, '\t')}
                    key={JSON.stringify(customizableAttributes)}
                  ...
                  <Button
                    ...
                    onClick={this._customizeModification}
