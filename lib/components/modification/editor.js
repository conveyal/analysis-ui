...
const useEmptyComponent = () => <React.Fragment />
const getComponentForType = type =>
  dynamic(() => import(`lib/containers/${type}`).catch(useEmptyComponent))

const ModificationType = props => {
  const M = getComponentForType(props.type)
  return M ? <M {...props} /> : <React.Fragment />
}
      ...
      <>
        ...
        {s.mapState && (
          <ModificationMapEdit
            ...
            setMapState={mapState => this.setState({mapState})}
