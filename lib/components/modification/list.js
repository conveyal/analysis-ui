...
import {RouteTo, LS_MOM} from 'lib/constants'
...
export default function ModificationsList(p) {
  ...
  const setAndStoreMoM = React.useCallback(
    newMoM => {
  ...
  return (
    <Application
      map={() => <ModificationsMap modificationsOnMap={modificationsOnMap} />}
      {...p}
    >
            ...
            >
              {ms.map(m => (
                <ModificationGroupItem
                  ...
                  regionId={regionId}
                  showOnMap={() =>
                    setModificationsOnMap(modificationsOnMap.concat(m._id))
                  }
                />
              ))}
            </ModificationGroup>
