...
export default class AddTripPattern extends DeepEqual {
  ...
  componentDidMount () {
    ...
    if (modification.segments.length > 0 &&
      modification.timetables.length > 0 &&
      modification.timetables.some((tt) => tt.segmentSpeeds.length !== modification.segments.length)) {
      this._setTimetableSegmentLengthsTo(modification.segments.length)
    }
  }

  componentWillReceiveProps (newProps) {
    if (newProps.modification.segments !== this.props.modification.segments) {
      this._setTimetableSegmentLengthsTo(newProps.modification.segments.length)
    }
  ...
  _setTimetableSegmentLengthsTo (newLength) {
    const {modification, update} = this.props
    update({
      timetables: modification.timetables.map((t) => {
        if (newLength <= t.segmentSpeeds.length) {
          return {
            ...t,
            segmentSpeeds: t.segmentSpeeds.slice(0, newLength)
          }
        } else {
          const segmentSpeeds = t.segmentSpeeds
          while (segmentSpeeds.length < newLength) segmentSpeeds.push(segmentSpeeds[0] || DEFAULT_SEGMENT_SPEED)
          return {
            ...t,
            segmentSpeeds
          }
  ...
  render () {
    const {modification, mapState, numberOfStops, update} = this.props
    const {extendFromEnd, followRoad, segmentDistances} = this.state
...
function getStateFromProps ({
  ...
  return {
    ...
    segmentDistances: numberOfStops > 1 ? modification.segments.map((s) => lineDistance(s.geometry, 'kilometers')) : [],
