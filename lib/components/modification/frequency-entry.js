...
export default class FrequencyEntry extends Component {
  ...
  render () {
    ...
    const patternsWithTrips = routePatterns.filter((pattern) =>
      !!pattern.trips.find((trip) =>
        !!entry.patternTrips.includes(trip.trip_id)))
    const stopsInPatterns = modificationStops.filter(({stop_id}) =>
      !!patternsWithTrips.find((pattern) =>
        !!pattern.stops.find((stop) =>
          stop.stop_id === stop_id.split(':')[1])))
    return (
      <section className='panel panel-default inner-panel' onFocus={this._setActiveTrips}>
        <a
