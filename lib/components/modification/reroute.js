...
export default class RerouteComponent extends Component<void, Props, void> {
  ...
  _clearFromStop = () => {
    ...
    p.update({
      fromStop: null,
      toStop: p.modification.segments.length === 1 ? null : p.modification.toStop,
  ...
  _clearToStop = () => {
    const p = this.props
    p.update({
      fromStop: p.modification.segments.length === 1 ? null : p.modification.fromStop,
      toStop: null,
  ...
  render () {
    ...
    const fromStopValue = get(p.selectedFeed, `stopsById[${p.modification.fromStop}].stop_name`, '(none)')
    const toStopValue = get(p.selectedFeed, `stopsById[${p.modification.toStop}].stop_name`, '(none)')
      ...
      <div>
        ...
        {hasFeedAndRoutes &&
          <div>
            ...
            <EditAlignment
              allowExtend={p.modification.toStop == null || p.modification.fromStop == null}
