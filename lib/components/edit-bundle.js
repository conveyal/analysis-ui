 ...
 */
function EditBundle(p) {
  const dispatch = useDispatch()
  const bundles = useSelector(s => s.region.bundles)
  const labeledBundles = labelBundles(bundles)

  const {bundleId, regionId} = p.router.query
  ...
  function selectBundle(result) {
    p.router.push({
      ...
      query: {regionId, bundleId: result.value}
    })
  }
