...
export default class Application extends Component {
  ...
  render () {
    const {activeModification, analysisMode, children, error, errorMessage, hasError, hasScenario, login, logout, mapComponents, outstandingRequests, projectId, userIsLoggedIn, username} = this.props
      ...
      <div id='editor'>
        ...
        <div
          ...
          <LeafletMap
            ...
            {hasScenario && <ScenarioMap zIndex={50} activeModification={activeModification} />}
          </LeafletMap>
        </div>

        <div className={`ApplicationDock ${analysisMode ? 'analysisMode' : ''}`}>
          <div className='ApplicationTitle'>
            <Link className='logo' to='/'>analysis </Link> {outstandingRequests > 0 && <Icon type='spinner' className='fa-spin' />}
