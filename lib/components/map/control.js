...
export default class Control extends Component {
  ...
  _fetchIsochrone = (latlng) => {
    const {bundleId, fetchIsochrone, isochroneLatLng, modifications, scenarioId} = this.props
    fetchIsochrone({
      bundleId,
      latlng: latlng || isochroneLatLng,
      modifications,
      scenarioId
    })
  }
  ...
  render () {
    ...
    return (
      <div className='MapControl'>
        ...
        {!isFetchingIsochrone && isShowingIsochrone &&
          <div className='block'>
            <Icon type='clock-o' /> <span>{isochroneCutoff / 60} minutes</span>
            <input
              className='pull-right'
