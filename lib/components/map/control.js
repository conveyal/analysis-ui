...
export default class Control extends Component {
  ...
  _fetchIsochrone = (lonlat) => {
    const {bundleId, fetchIsochrone, isochroneLonLat, modifications, scenarioId, isochroneCutoff} = this.props
    fetchIsochrone({
      bundleId,
  ...
  _setIsochroneCutoff = (e) => {
    const {bundleId, isochroneLonLat, modifications, scenarioId, setIsochroneCutoff, fetchIsochrone} = this.props
  ...
  _toggleIsochroneLayer = (e) => {
    console.log()
    e.preventDefault()
    const {addIsochroneLayerToMap, isShowingIsochrone, removeIsochroneLayerFromMap, addOpportunityLayerToMap, removeOpportunityLayerFromMap} = this.props
    if (isShowingIsochrone) {
  ...
  render () {
    const {center, geocoderApiKey, isFetchingIsochrone, isochroneCutoff, isShowingIsochrone, accessibility} = this.props
    ...
    return (
      <div className='MapControl'>
        ...
        {isShowingIsochrone && <span>{accessibility} jobs accessible</span>}
