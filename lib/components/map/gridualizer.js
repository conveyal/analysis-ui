...
function createCreateTile(props) {
  ...
  if (props.grid) {
    drawTile = createDrawTile({
      colorizer:
        props.colorizer || colorizers.choropleth(props.breaks, props.colors),
      grid: props.grid,
...
export class GridualizerLayer extends GridLayer {
  createLeafletElement(props) {
    const gridLayer = super.createLeafletElement({
      ...props,
      zIndex: 300 // to position above the tile pane
    })
