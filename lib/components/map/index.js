...
function Map(p) {
  const [center, setCenter] = React.useState(p.center)

  // Keep props center and state center in sync but prevent unnecessary renders
  React.useEffect(() => {
    if (!center || !lonlat.isEqual(p.center, center)) {
      setCenter(p.center)
    }
  }, [p.center])
