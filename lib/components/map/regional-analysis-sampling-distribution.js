 ...
 */
export default class RegionalAnalysisSamplingDistribution extends Component<void, Props, State> {
  ...
  render () {
    const {comparisonSamplingDistribution, origin, remove, samplingDistribution} = this.props
    ...
    return <DraggablePopup position={origin} dragEnd={this.dragEnd} remove={remove} minWidth={WIDTH}>
      <svg width={WIDTH} height={HEIGHT}>
        <KernelDensity color={SCENARIO_PERCENTILE_COLOR} estimate={estimate} xScale={xScale} yScale={yScale} />
        <PointEstimate color={SCENARIO_PERCENTILE_COLOR} x={xScale(samplingDistribution[0])} />

        {comparisonSamplingDistribution && comparisonEstimate &&
          <KernelDensity color={COMPARISON_PERCENTILE_COLOR} estimate={comparisonEstimate} xScale={xScale} yScale={yScale} />}
        {comparisonSamplingDistribution &&
          <PointEstimate color={COMPARISON_PERCENTILE_COLOR} x={xScale(comparisonSamplingDistribution[0])} />}
...
function precomputeScales ({
  ...
  const estimate = stats.kde()
    .sample(samplingDistribution)(range(WIDTH).map(xScale.invert))

  const comparisonEstimate = comparisonSamplingDistribution
    ? stats.kde().sample(comparisonSamplingDistribution)(range(WIDTH).map(xScale.invert))
    : null
