...
export default class Isochrone extends Component {
  ...
  render () {
    const {isochrone, isochroneCutoff, isochroneLatLng, comparisonIsochrone, doingComparison} = this.props
    return (
      <g>
        <Marker
          draggable
        ...
        {doingComparison && comparisonIsochrone &&
          <GeoJson
            ...
            key={`isochrone-comparison-${isochroneCutoff}-${lonlng.toString(isochroneLatLng)}`}
            style={{
              fillColor: process.env.COMPARISON_ISOCHRONE_COLOR,
              pointerEvents: 'none',
              stroke: false
            }}
          />}
        ...
        {isochrone &&
          <GeoJson
            ...
            style={{
              fillColor: process.env.SCENARIO_ISOCHRONE_COLOR,
              pointerEvents: 'none',
              stroke: false
            }}
            />}
