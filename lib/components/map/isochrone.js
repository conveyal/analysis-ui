...
export default class Isochrone extends Component {
  ...
  render () {
    const {isochrone, isochroneCutoff, isochroneLatLng, comparisonIsochrone, isFetchingIsochrone, doingComparison} = this.props
    return (
      <g>
        <Marker
          draggable
        ...
        {doingComparison && comparisonIsochrone &&
          <GeoJson
            ...
            style={{
              fillColor: isFetchingIsochrone
                ? process.env.STALE_ISOCHRONE_COLOR
                : process.env.COMPARISON_ISOCHRONE_COLOR,
              pointerEvents: 'none',
              stroke: false
            }}
          />}
        ...
        {isochrone &&
          <GeoJson
            ...
            style={{
              fillColor: isFetchingIsochrone
                ? process.env.STALE_ISOCHRONE_COLOR
                : process.env.SCENARIO_ISOCHRONE_COLOR,
              pointerEvents: 'none',
              stroke: false
            }}
            />}
  ...
  _fetchIsochrone = (latlng) => {
    const {bundleId, fetchIsochrone, modifications, scenarioId, variantIndex, isochroneCutoff, currentIndicator, workerVersion, comparisonScenarioId, comparisonVariant, comparisonBundleId, comparisonModifications, doingComparison} = this.props
