...
export default class Variants extends Component<void, Props, void> {
  _create = () => {
    ...
    const variantName = window.prompt('Enter a variant name:', `Variant ${variants.length + 1}`)
  ...
  _deleteVariant = memoize((index: number) => () => {
    ...
    if (window.confirm(`Are you sure you want to delete variant ${name}? This will clear all references to this variant from each modification.`)) {
      deleteVariant(index)
    }
  })
  ...
  _editVariantName = memoize((index: number) => () => {
    ...
    const newVariantName = window.prompt('Enter a new variant name:', variantName)
