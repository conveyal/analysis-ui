...
export default class EditScenario extends PureComponent {
  ...
  _createOrSave = () => {
    const {close, create, _id, isEditing, regionId, variants, save} = this.props
    const {bundleId, name} = this.state
    if (name && bundleId) {
      if (isEditing) {
        save({bundleId, _id, name, regionId, variants})
        close()
      } else {
        create({
          bundleId,
  ...
  render () {
    ...
    return (
      <InnerDock>
        <div className='block'>
          <h5>
            {isEditing ? messages.scenario.editTitle : messages.scenario.createAction}
          </h5>
          <Text
            name={messages.scenario.name}
          ...
          {!isEditing &&
            <div>
              <p>{messages.scenario.createActionTooltip}</p>
              {bundles.length > 0
              ...
              : <p>{messages.scenario.noBundles}</p>}
              <FormGroup>
