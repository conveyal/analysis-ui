  ...
  componentDidMount () {
    ...
    fetch(R5_BUCKET)
      ...
      .then(res => {
        ...
        listing.sort((a, b) => {
          let [, amajor, aminor, apatch, acommit, adescribe] = a.match(VERSION_PARSE_REGEX)
          let [, bmajor, bminor, bpatch, bcommit, bdescribe] = b.match(VERSION_PARSE_REGEX)
          ...
          acommit = parseInt(acommit)
          bcommit = parseInt(bcommit)

          if (acommit - bcommit < 0) return 1
          else if (acommit - bcommit > 0) return -1
