...
export const Button = pure(function Button ({block, children, className, onClick, size, style, href = '#', target, checked, radio, ...props}) {
  ...
  if (radio) {
    const id = props.id || `radio-button-${idCounter++}`
    // labels cannot contain children, so group with a span and link with the id
    // TODO what to do with the onClick here? put it on the label or the form element?
