...
type Props = {
  ...
  downloadLines: (index: number) => void,
  downloadStops: (index: number) => void,
  project?: {
    _id: string,
    bundleId: string,
    name: string,
  ...
  _export = (index: number, format: string) => {
    switch (format) {
      case 'json':
        ...
        break
      case 'lines':
        this.props.downloadLines(index)
        break
      case 'stops':
        this.props.downloadStops(index)
        break
    }
  }
        ...
        {scenarios.map((name, index) => (
          <div key={`export-${index}`}>
            ...
            <ButtonGroup justified>
              <Button
                style='info'
                onClick={() => exportAction(index, 'lines')}
            >
                <Icon type='download' /> {message('variant.saveGeojson')}
              </Button>
              <Button
                style='info'
                onClick={() => exportAction(index, 'stops')}
            >
                <Icon type='download' /> {message('variant.saveStops')}
              </Button>
            </ButtonGroup>
          </div>
        ))}
