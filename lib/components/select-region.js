...
const getAdminAccessGroup = s => s.user.adminTempAccessGroup

export default function SelectRegion(p) {
  const adminAccessGroup = useSelector(getAdminAccessGroup)
  ...
  return (
    <Container>
      ...
      {adminAccessGroup && (
        <p className='center'>
          signed in as <strong>{adminAccessGroup}</strong>
        </p>
      )}
      <p>
