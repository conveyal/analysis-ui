...
export function ErrorModal(p) {
  const {error, errorMessage, url, stack} = p
  return !error ? null : (
    <Modal onRequestClose={p.clearError}>
      <ModalTitle>
        {message('error.title')}
        <Button onClick={p.clearError} style='danger' className='pull-right'>
          {message('error.close')}
        </Button>
      </ModalTitle>
      <ModalBody>
        ...
        {p.url && <p>{url}</p>}
        {p.stack && (
          <Collapsible title={message('error.details')}>
