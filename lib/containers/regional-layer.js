...
function mapStateToProps ({ analysis, project, mapState }) {
  return {
    // TODO hack
    baseGrid: analysis.regional.grid || { data: [], width: 0, height: 0, north: 0, west: 0, zoom: 9 },
    ...
    id: analysis.regional.id,
    isRegionalAnalysisSamplingDistributionComponentOnMap: mapState.components.indexOf(REGIONAL_ANALYSIS_SAMPLING_DISTRIBUTION_COMPONENT) > -1
  }
}

const mapDispatchToProps = function (dispatch) {
  return {
    removeRegionalAnalysisSamplingDistributionComponentFromMap: () => dispatch(removeComponent(REGIONAL_ANALYSIS_SAMPLING_DISTRIBUTION_COMPONENT)),
    setRegionalAnalysisOrigin: origin => dispatch(setRegionalAnalysisOrigin(origin))
  }
}
