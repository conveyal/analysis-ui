...
describe('Container > Modification Editor', () => {
  it('renders and can change the name', async () => {
    // mount component
    const p = testComponent(ModificationEditor, {query})
    const wrapper = p.mount()

    wrapper.find('a[name="Edit modification name"]').simulate('click')

    wrapper
      .find('input[name="Modification Name"]')
      .simulate('change', {currentTarget: {value: 'New Modification Name'}})

    expect(p.store.getActions()).toMatchSnapshot()
    wrapper.unmount()
  })
  ...
  it('render just the title if not loaded', async () => {
    ...
    const p = testComponent(ModificationEditor, {query}, init)
    const wrapper = p.mount()
