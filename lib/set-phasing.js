...
const GTFS_API_URL = 'http://localhost:4567'
...
export default class SetTripPhasing extends Component {
  constructor (props) {
    ...
    let routePromise = fetch(`${GTFS_API_URL}/routes?feed=${FEED_ID}`).then(res => res.json())
    let tripPromise = fetch(`${GTFS_API_URL}/trips?feed=${FEED_ID}`).then(res => res.json())
    let stopPromise = fetch(`${GTFS_API_URL}/stops?feed=${FEED_ID}`).then(res => res.json())

    Promise.all([routePromise, tripPromise, stopPromise]).then(([routes, trips, stops]) => {
      Promise.all(
        trips.map(t =>
          fetch(`${GTFS_API_URL}/trips/${t.trip_id}/stoptimes?feed=${FEED_ID}`).then(res => res.json())
        )
      ).then(stopTimeArray => {
