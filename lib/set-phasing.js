...
export default class SetTripPhasing extends Component {
  ...
  constructor (props) {
    ...
    let routePromise = authenticatedFetch(`${GTFS_API_URL}/routes?feed=${FEED_ID}`).then(parseJSON)
    let tripPromise = authenticatedFetch(`${GTFS_API_URL}/trips?feed=${FEED_ID}`).then(parseJSON)
    let stopPromise = authenticatedFetch(`${GTFS_API_URL}/stops?feed=${FEED_ID}`).then(parseJSON)

    Promise.all([routePromise, tripPromise, stopPromise]).then(([routes, trips, stops]) => {
      Promise.all(
        trips.map((t) =>
          authenticatedFetch(`${GTFS_API_URL}/trips/${t.trip_id}/stoptimes?feed=${FEED_ID}`).then(parseJSON)
        )
      ).then((stopTimeArray) => {
