...
export default class AddTripPattern extends Component {
  ...
  constructor (props) {
    ...
    this.state = {
      editing: false,
      // this is a clooge to get around the fact that as the user is typing a change to stop spacing the map will otherwise
      // try to render stops every 1 meter if you start typing 1000. So we store it in state and only update the map when the number
      // is large enough to be reasonable
      spacing: this.getSpacingFromModification(this.props.modification)
    }
  }

  /** extract current stop spacing from a modification */
  getSpacingFromModification (modification) {
    return modification.segments.length > 0 ? modification.segments[0].spacing : 400
  }

  componentWillReceiveProps (newProps) {
    this.setState(Object.assign({}, this.state, { spacing: this.getSpacingFromModification(newProps.modification) }))
  }
