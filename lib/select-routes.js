  ...
  render () {
    return <div>
      <select value={this.props.feed} onChange={this.selectFeed}>
        {this.state.feeds.map((f) => <option value={f} key={f}>{f}</option>)}
      </select><br/>

      <select value={this.props.routes ? this.props.routes[0] : undefined} onChange={this.selectRoute}>
        <option></option>
        {this.state.routes.map((r) => <option value={r.route_id} key={r.route_id}>{(r.route_short_name ? r.route_short_name + ' ' : '') + (r.route_long_name ? r.route_long_name : '')}</option>)}
      </select>
    </div>
  }
}
