      ...
      {(() => {
        if (this.props.feed) {
          return <select value={this.props.routes ? this.props.routes[0] : undefined} onChange={this.selectRoute}>
            <option></option>
            {[...this.props.data.get(this.props.feed).values()].map((r) => <option value={r.route_id} key={r.route_id}>{(r.route_short_name ? r.route_short_name + ' ' : '') + (r.route_long_name ? r.route_long_name : '')}</option>)}
          </select>
        } else return <span/>
      })()}
