  ...
  render () {
    return <div>
      <select value={this.props.feed} onChange={this.selectFeed}>
        <option></option>
        {[...this.props.data.keys()].map((f) => <option value={f} key={f}>{f}</option>)}
      </select><br/>

      {(() => {
        // display if the user has chosen a feed, and data for that feed has loaded
        if (this.props.feed && this.props.data.has(this.props.feed)) {
          return <select value={this.props.routes ? this.props.routes[0] : undefined} onChange={this.selectRoute}>
            <option></option>
            {[...this.props.data.get(this.props.feed).routes.values()].map((r) => <option value={r.route_id} key={r.route_id}>{(r.route_short_name ? r.route_short_name + ' ' : '') + (r.route_long_name ? r.route_long_name : '')}</option>)}
          </select>
        } else return <span/>
      })()}
