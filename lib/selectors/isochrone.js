...
function computeIsochrone (singleValuedSurface, isochroneCutoff) {
  ...
  return jsolines({
    ...
    project: ([x, y]) => {
      const {lat, lng} = LeafletMap.prototype.unproject([x + west, y + north], zoom)
      return [lng, lat]
    }
  })
}
 ...
 */
function computeSingleValuedSurface (travelTimeSurface) {
