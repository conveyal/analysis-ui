...
export default createSelector(
  ...
  (lonlat, qs, requestSettings, resultsSettings) => {
    // If the results settings contains a different amount of entries than the current settings
    if (requestSettings.length !== resultsSettings.length) return true
      ...
      hasChanged(
        {
          ...
          variantIndex: parseInt(qs.variantIndex)
        },
        resultsSettings[0]
      )
    )
      ...
      hasChanged(
        {
          ...
          variantIndex: parseInt(qs.comparisonVariantIndex)
        },
        resultsSettings[1]
      )
    )
