 ...
 */
const getStopsFromTrip = (feed, m) => feed.routes
      .find(r => r.route_id === m.routes[0]).patterns
      .find(patternTripsInModification([m.sourceTrip])).stops
      .map(s => s.stop_id)
  ...
  (modifications, feeds) => {
    ...
    Object.keys(modifications).forEach(key => {
      ...
      if (modification.entries) { // get stops from a referenced adjust-frequency mod (which has entries)
        ...
        if (!feed) return
        stopsForTimetable[key] = getStopsFromTrip(
          feed,
          {
            ...modification,
