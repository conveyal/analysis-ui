...
export default createSelector(
  [selectPhaseModifications, state => state.project.feedsById],
  (modifications, feeds) => {
    ...
    Object.keys(modifications).forEach(key => {
      ...
      if (modification.entries) { // get stops from a referenced adjust-frequency mod (which has entries)
        ...
        if (!feed) return
        const modificationPatterns = feed.routes.find(r => r.route_id === modification.routes[0]).patterns
        // Get stops of a trip in a referenced modification
        // (not the active modification, which is why we don't use a selector)
        const sourceTripStops = modificationPatterns.find(patternTripsInModification([entry.sourceTrip])).stops
        stopsForTimetable[key] = sourceTripStops.map(s => s.stop_id)
