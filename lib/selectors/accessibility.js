...
const POSITION = 9 // ninth (zero-based) item in an array of 19 percentiles, 5th to 95th is the 50th

export function computeAccessibility (travelTimeSurface, isochroneCutoff, destinationGrid) {
  ...
  for (let y = 0; y < destinationGrid.height; y++) {
    ...
    for (let x = 0; x < destinationGrid.width; x++) {
      ...
      // less than is correct here, times are floored on the server when converted from seconds
      // to minutes, so a travel time of 59m59s will have a value of 59, not 60.
...
const accessibility = createSelector(
