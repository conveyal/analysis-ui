 ...
 */
export function getAggregateAccessibility (accessibility: Grid, aggregationArea: Grid, weights: Grid): void | AggregateAccessibility {
  if (weights == null || aggregationArea == null || accessibility == null) return

  if (accessibility.zoom !== weights.zoom || accessibility.zoom !== aggregationArea.zoom) {
  ...
  for (let y = 0, pixel = 0; y < aggregationArea.height; y++) {
    ...
    for (let x = 0; x < aggregationArea.width; x++, pixel++) {
      if (aggregationArea.data[pixel] > 0) {
        ...
        const aggregationAreaWeightThisCell = aggregationArea.data[pixel] / 100000 // how much of this cell is within the aggregationArea
