...
import type {Grid, GeoJsonFeature} from '../types'

// cut off at cells that are 50% covered by the aggregation area. 30000 would be 30% etc
const CUTOFF = 50000

export function computeOutline (aggregationArea: Grid): GeoJsonFeature {
  ...
  return jsolines({
    ...
    project: ([x, y]) => {
      ...
      const {lat, lng} = LeafletMap.prototype.unproject([x + west - 1, y + north - 1], zoom)
      return [lng, lat]
    },
