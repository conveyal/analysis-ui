...
const patternTripsInModification = modificationTrips => pattern =>
  pattern.trips.find(t => modificationTrips.indexOf(t.trip_id) > -1)

const uniqueStopIds = patterns => [
  ...new Set(
    patterns.reduce(
      (stopIds, pattern) => [...stopIds, ...pattern.stops.map(s => s.stop_id)],
      []
    )
  )
]

const getStopFromFeed = feedStopsById => stopId => feedStopsById[stopId]
