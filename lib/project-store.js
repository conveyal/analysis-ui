...
class ProjectStore {
  ...
  saveProject (projectId, project) {
    ...
    promises.push(fetch(`/scenario/${project.id}`, {
      method: 'put',
      body: JSON.stringify(projectToStore)
    }))

    project.modifications.forEach((m, id) => {
      promises.push(fetch(`/modification/${m.id}`, {
        method: 'put',
  ...
  getProject (projectId) {
    let projectPromise = fetch(`/scenario/${projectId}`).then(res => res.json())
    let modsPromise = fetch(`/scenario/${projectId}/modifications`).then(res => res.json())
  ...
  getProjects () {
    return fetch(`/scenario`).then(res => res.json())
  }
}
