...
class ProjectStore {
  ...
  saveProject (projectId, project) {
    ...
    promises.push(fetch(`/api/scenario/${project.id}`, {
      method: 'put',
      body: JSON.stringify(projectToStore)
    }))

    project.modifications.forEach((m, id) => {
      promises.push(fetch(`/api/modification/${m.id}`, {
        method: 'put',
  ...
  getProject (projectId) {
    let projectPromise = fetch(`/api/scenario/${projectId}`).then(res => res.json())
    let modsPromise = fetch(`/api/scenario/${projectId}/modifications`).then(res => res.json())
  ...
  getProjects () {
    return fetch('/api/scenario').then(res => res.json())
  }
}
