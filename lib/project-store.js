...
class ProjectStore {
  constructor () {
    store.subscribe(() => {
      ...
      this.saveProject(state.projectId, state.modifications)
    })
  }

  saveProject (projectId, modifications) {
    ...
    // store the project
    window.localStorage.setItem(`proj-${projectId}`, JSON.stringify(modifications))
  }
