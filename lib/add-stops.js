...
export default class AddStops extends Component {
  ...
  setSpeed = (e) => {
    let mod = Object.assign({}, this.props.modification, { speed: parseInt(e.target.value) })
    this.props.replaceModification(mod)
  }

  setDwell = (e) => {
    let mod = Object.assign({}, this.props.modification, { dwell: parseInt(e.target.value) })
    this.props.replaceModification(mod)
  }
  ...
  renderStops () {
    ...
    if (modification.feed && modification.routes) {
      const feed = this.props.data.feeds.get(modification.feed)
      if (feed) {
