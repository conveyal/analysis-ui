...
export default class AddStops extends Component {
  ...
  onSelectorChange = ({feed, routes, trips}) => {
    this.props.replaceModification(Object.assign({}, this.props.modification, {feed, routes, trips}))
  }
  ...
  render () {
    const {feeds, feedsById, modification, mapState} = this.props
  ...
  renderStops () {
    ...
    if (modification.feed && modification.routes) {
      const feed = this.props.feedsById[modification.feed]
      if (feed) {
        const fromStopLabel = `From stop: ${modification.fromStop ? feed.stopsById[modification.fromStop].stop_name : '(none)'}`
        const toStopLabel = `To stop: ${modification.toStop ? feed.stopsById[modification.toStop].stop_name : '(none)'}`
        return (
          <div>
