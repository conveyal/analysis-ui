...
const setMaskLocally = createAction('set mask')
const setMaskId = createAction('set mask id')
export const setMask = async ({projectId, maskId}) => [
  setMaskLocally(null), // clear existing so UI doesn't show results that don't match now-selected mask
  setMaskId(maskId),
  fetch({
    url: `/api/project/${projectId}/mask/${maskId}?redirect=false`,
    next: (err, res) => !err && fetch({
      url: res.value.url,
      headers: {
        Authorization: null // don't pass our auth header through to S3, causes 401 err
      },
      next: (err, res) => !err && setMaskLocally(createGrid(res.value))
    })
  })
]

const setMaskWeightsLocally = createAction('set mask weights')
const setMaskWeightsId = createAction('set mask weights id')
export const setMaskWeights = ({projectId, maskWeightsId}) => [
  setMaskWeightsLocally(null), // clear existing so UI doesn't show results that don't match now-selected mask
  setMaskWeightsId(maskWeightsId),
  fetch({
    url: `/api/grid/${projectId}/${maskWeightsId}?redirect=false`,
    next: (err, res) => !err && fetch({
      url: res.value.url,
      headers: {
        Authorization: null // don't pass our auth header through to S3, causes 401 err
      },
      next: (err, res) => !err && setMaskWeightsLocally(createGrid(res.value))
    })
  })
]

const setMaskUploading = createAction('set mask uploading')
/** This triggers a reducer in project to inject the mask into the current project */
const addMaskLocally = createAction('add mask locally')

/** Upload a mask */
export const uploadMask = ({name, files, projectId}) => {
  ...
  return [
    ...
    fetch({
      url: `/api/project/${projectId}/mask`,
      options: {
        method: 'post',
        body: formData
      },
      next: (err, res) => !err && [addMaskLocally(res.value), setMaskUploading(false)]
    })
  ]
}
