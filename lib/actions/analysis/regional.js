...
export const loadRegionalAnalysisGrids = async ({
  ...
  if (comparisonId) {
    return [
      ...
      Promise.all(
        [
          ...
          // or grid/tiff
          authenticatedFetch(`/api/regional/${id}/grid/grid`).catch(res => {
            console.error(res)
            return res
          }),
          authenticatedFetch(`/api/regional/${comparisonId}/grid/grid`).catch(res => {
            console.error(res)
            return res
          }),
          // comparison query is the _base_, i.e. what is subtracted
          authenticatedFetch(`/api/regional/${comparisonId}/${id}/grid`).catch(res => {
            console.error(res)
            return res
          })
        ]
    ...
    return [
      ...
      fetch({
        url: `/api/regional/${id}/grid/grid`,
        next: async (err, response) => {
          let gridData
          if (err) {
            const res2 = await window.fetch(response.url)
            gridData = await res2.arrayBuffer()
          } else {
            gridData = response.value
          }
...
export const createRegionalAnalysis = ({
  ...
  const {analysis, project, scenario} = getState()
  const {north, south, east, west} = bounds
  const {currentIndicator, isochroneCutoff} = analysis
  ...
  dispatch(fetch({
    ...
    url: '/api/regional',
    next: (error) => !error && push(`/scenarios/${currentScenario.id}/regional`)
  }))
}
