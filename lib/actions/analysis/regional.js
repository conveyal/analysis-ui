...
export const loadRegionalAnalysisGrids = async ({
  ...
  if (comparisonId) {
    return [
      ...
      Promise.all(
        [
          ...
          // or grid/tiff
          authenticatedFetch(`${REGIONAL_URL}/${_id}/grid/grid`).catch(res => {
            console.error(res)
            return res
          }),
          authenticatedFetch(
    ...
    return [
      ...
      authenticatedFetch(`${REGIONAL_URL}/${_id}/grid/grid`)
        .catch(res => res)
...
export const setAggregationArea = async ({
  ...
  fetch({
    url: `/api/region/${regionId}/aggregationArea/${aggregationAreaId}?redirect=false`,
    next: (err, res) =>
      !err &&
      fetch({
        url: res.value.url,
...
export const setAggregationWeights = ({
  ...
  fetch({
    url: `/api/grid/${regionId}/${aggregationWeightsId}?redirect=false`,
    next: (err, res) =>
      !err &&
      fetch({
        url: res.value.url,
