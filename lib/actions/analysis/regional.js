...
import type {LatLon, RegionalAnalysis} from '../../types'
...
const setRegionalAnalysisOriginLocally = createAction('set regional analysis origin')
const setRegionalAnalysisSamplingDistribution = createAction('set regional analysis sampling distribution')
const setComparisonRegionalAnalysisSamplingDistribution = createAction('set comparison regional analysis sampling distribution')
  ...
  if (comparisonId) {
    return [
      ...
      .then(([ gridRaw, comparisonGridRaw, probabilityGridRaw ]) => {
        ...
        const breaks = classifiers.diverging({scheme: classifiers.quantile})(differenceGrid, colors.REGIONAL_COMPARISON_GRADIENT.length)
    ...
    return [
      ...
      authenticatedFetch(`/api/regional/${id}/grid/grid`)
        ...
        .then(async (gridData) => {
          ...
          const breaks = classifiers.equal({
...
export const runAnalysis = (analysis: RegionalAnalysis) => serverAction({
  url: `/api/regional`,
  options: {
    method: 'POST',
...
export const setRegionalAnalysisOrigin = async ({
  ...
  const fetches = [{
    url: `/api/regional/${regionalAnalysisId}/samplingDistribution/${latlon.lat}/${latlon.lon}`
  }]

  if (comparisonRegionalAnalysisId) {
    fetches.push({
      url: `/api/regional/${comparisonRegionalAnalysisId}/samplingDistribution/${latlon.lat}/${latlon.lon}`
    })
  }
  ...
  return fetchMultiple({
    ...
    next: (err, response) => {
      ...
      if (!err) {
        return [
          setRegionalAnalysisOriginLocally(latlon),
          setRegionalAnalysisSamplingDistribution(response[0].value),
          setComparisonRegionalAnalysisSamplingDistribution(comparisonRegionalAnalysisId ? response[1].value : null),
