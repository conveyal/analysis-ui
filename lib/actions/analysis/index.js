  ...
  (dispatch: Dispatch, getState: GetState) => {
    ...
    async function retry (response) {
      if (response.status === 202 && ++retryCount < MAX_RETRIES) {
        dispatch(setIsochroneFetchStatus(INITIALIZING_CLUSTER))
 ...
 */
const handleGeoTIFF = (name, comparisonName) => (responses) => {
  downloadGeoTIFF({
  ...
  if (responses[1] && responses[1].value) {
    downloadGeoTIFF({
      ...
      filename: `analysis-geotiff-${comparisonName}.geotiff`
    })
  }
...
export const handleSurface = (profileRequest: any) => (error: any, responses: any) => {
  if (error) {
    return [
      ...
      setScenarioApplicationErrors(Array.isArray(responses.value)
        ? responses.value
        : [responses.value])
    ]
  }
