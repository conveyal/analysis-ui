    ...
    next: async response => {
      ...
      return setComparisonModifications({
        ...
        modifications:
          variantIndex === -1
            ? []
            : (await response.json()).filter(m => m.variants[variantIndex])
      })
    }
  })
]

export const setCurrentIndicator = (projectId, indicator) =>
  fetch({
      ...
      if (!err) {
        return fetch({
          ...
          next: (error, res) => {
            if (!error) {
              return [
                setIndicatorLocally(indicator),
                setDestinationGrid(createGrid(res.value))
              ]
            }
          }
        })
...
export const fetchTravelTimeSurface = ({
  ...
  const {analysis, mapState, project, scenario} = state
  ...
  dispatch(doTravelTimeSurfaceFetch({
    ...
    scenario: {
      ...
      modifications: scenario.modifications
        .filter((m) => m.variants[variantIndex]),
...
const doTravelTimeSurfaceFetch = async ({
  scenario,
  comparison,
  bounds,
  dispatch,
  ...
  if (comparison) {
    requests.push(buildRequest({...comparison, ...projectedBounds}))
  }
  ...
  return fetchMultiple({
    ...
    next: async (error, responses) => {
      // Errors that are caused by malformed scenarios have status 400 and are specially handled below
      // The default lock ui fetch error action is also disabled for these errors specifically in reducers/network.
      // NB if there is an error this function will be called
      if (error && error.status === 400) {
        return [
          // responses is actually just the single response when there was an error
