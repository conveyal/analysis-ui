...
import {ANALYSIS_URL, FETCH_TRAVEL_TIME_SURFACE} from 'lib/constants'
...
export const fetchGeoTIFF = (requestSettings) => (dispatch, getState) => {
  dispatch(
 ...
 */
export const fetchTravelTimeSurface = (requestsSettings) => (
  dispatch,
  getState
) => {
  const state = getState()
  ...
  const profileRequests = [
    {
      ...
      fromLat: fromLonLat.lat,
      fromLon: fromLonLat.lon,
      projectId: get(state, 'queryString.projectId'),
      variantIndex: parseInt(get(state, 'queryString.variantIndex', -1))
    }
  ]

  const comparisonProjectId = get(state, 'queryString.comparisonProjectId')
  if (comparisonProjectId) {
    profileRequests.push({
      ...(requestsSettings[1] || requestsSettings[0]), // will not exist if copying
      fromLat: fromLonLat.lat,
      fromLon: fromLonLat.lon,
      projectId: comparisonProjectId,
      variantIndex: parseInt(
        get(state, 'queryString.comparisonVariantIndex', -1)
      )
    })
  }
...
export const clearTravelTimeSurfaces = () => [
