...
function buildRequest ({ bundleId, scenarioId, variantIndex, modifications, workerVersion, profileRequest, zoom, north, west, width, height }) {
  ...
  return {
    ...
    // AnalysisRequest objects now extend ProfileRequest rather than wrapping it
    // thus comment out code that unwraps ProfileRequests.
    // TODO handle this directly in UI; @trevorgerhardt mentioned he wanted to use a single object
    // to represent all analysis state. That should be fairly straightforward now. See #443.
    // request: {
    ...profileRequest,
    scenario: {
