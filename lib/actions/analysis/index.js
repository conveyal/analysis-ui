      ...
      next (error, responses) {
        if (error) {
          ...
          downloadGeoTIFF({
            ...
            filename: `analysis-geotiff-${project ? project.name : ''}-${profileRequest.variantIndex}-${profileRequest.maxTripDurationMinutes}-minutes.geotiff`
          })
          if (comparisonInProgress && responses[1]) {
            downloadGeoTIFF({
              data: responses[1].value,
              filename: `analysis-geotiff-${state.analysis.comparisonProjectId}-${state.analysis.comparisonVariant}-${profileRequest.maxTripDurationMinutes}-minutes.geotiff`
            })
          }
