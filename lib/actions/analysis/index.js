...
const setDestinationGrid = createAction('set desination grid')
...
export const setCurrentIndicator = (projectId, indicator) => fetch({
  ...
  next: (err, res) => {
    if (err) {
      ...
      return [setIndicatorLocally(indicator), setDestinationGrid(createGrid(res))]
    }
  }
})

export const fetchTravelTimeSurface = ({ scenario, comparison, bounds, dispatch }) => [
  setIsochroneFetchStatus(true),
  doTravelTimeSurfaceFetch({ scenario, comparison, bounds, dispatch })
]

const doTravelTimeSurfaceFetch = async ({ scenario, comparison, bounds, dispatch }) => {
