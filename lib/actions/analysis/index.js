    ...
    if (err) {
      ...
      return fetch({
        ...
        next: (error, res) => {
          ...
          else return [setIndicatorLocally(indicator), setDestinationGrid(createGrid(res.value))]
        }
      })
    }
  }
...
function responseToSurface (response) {
  if (response[0] && response[0].title) {
    ...
    const decoder = new window.TextDecoder('utf-8') // utf-8 is Jackson default
