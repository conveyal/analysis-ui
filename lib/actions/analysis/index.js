...
import {fetchMultiple, FETCH_ERROR} from 'lib/fetch-action'
...
export const fetchTravelTimeSurface = asGeoTIFF => (dispatch, getState) => {
  ...
  const ownProps = {query: Router.query}

  const state = getState()
  const project = select.currentProject(state, ownProps)
  const currentProjectId = select.currentProjectId(state, ownProps)
  const comparisonInProgress = select.comparisonInProgress(state, ownProps)
  const comparisonProject = select.comparisonProject(state, ownProps)
  const workerVersion = R5Version.select.currentVersion(state, ownProps)
...
const handleGeoTIFF = names => responses => {
  return (
    Promise
      ...
      .all(responses.map(r => r.value || r.arrayBuffer()))
      .then(values => {
 ...
 */
export const handleSurface = profileRequest => (error, responses) => {
  if (
