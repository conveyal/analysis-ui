...
export const setComparisonProjectId = createAction('set comparison project')
...
/** Sets the comparison project ID, and retrieves that project */
export const setComparisonProject = ({_id, bundleId, variantIndex}: {
  _id: string,
  bundleId: string,
  variantIndex: number
}) => [
  setComparisonProjectId({_id, bundleId, variantIndex}),
  fetch({
    url: `/api/project/${_id}/modifications`,
    next: (err, response) =>
      !err &&
    ...
    dispatch(
      doTravelTimeSurfaceFetch({
        ...
        project: {
          ...
          modifications: project.modifications.filter(
        ...
        comparison: !selectComparisonInProgress(state)
          ...
          : {
            ...
            projectId: analysis.comparisonProjectId,
...
const doTravelTimeSurfaceFetch = ({project, comparison, bounds}) =>
  (dispatch: Dispatch, getState: GetState) => {
    ...
    const requests = []
    requests.push(buildRequest({...project, ...regionedBounds}))
    if (comparison) {
    ...
    dispatch(
      fetchMultiple({
        ...
        next: async (error, responses) => {
          // Errors that are caused by malformed projects have status 400 and are
          ...
          if (error && error.status === 400) {
            return [
              ...
              setProjectApplicationErrors(responses.value),
            ...
            let [projectSurface, comparisonSurface] = responses.map(i =>
            ...
            return [
              ...
              setProjectApplicationWarnings([
                ...projectSurface.warnings,
                ...(comparisonSurface ? comparisonSurface.warnings : [])
              ]),
              setTravelTimeSurface(projectSurface),
              ...
              setIsochroneLonLat({
                lat: project.profileRequest.fromLat,
                lon: project.profileRequest.fromLon
              }),
              setProfileRequest(project.profileRequest)
            ]
          }
        }
      })
...
function buildRequest ({
  ...
  // ensure project reapplication when edited
  const projectCrc = crc32(JSON.stringify(modifications))
  ...
  return {
    ...
    project: {
      id: `${projectId}-${variantIndex}-${projectCrc}`,
