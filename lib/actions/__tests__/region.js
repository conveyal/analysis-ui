...
describe('actions > region', () => {
  ...
  it('create should work', async () => {
    ...
    const create = region.create({region: mockRegion})
  ...
  it('deleteRegion should work', async () => {
    nock('http://mockHost.com/').delete('/api/region/1').reply(200)
  ...
  it('load should work', async () => {
    ...
    nock('http://mockHost.com/').get('/api/region/1').reply(200, mockRegion)
  ...
  it('save should work', async () => {
    nock('http://mockHost.com/').put('/api/region/1').reply(200, mockRegion)

    const store = makeMockStore()
    const action = region.save({region: mockRegion})
    expect(action).toMatchSnapshot()
    const actionResult = store.dispatch(action)
    expect(actionResult).toMatchSnapshot()
    const fetchResult = await actionResult[1]
