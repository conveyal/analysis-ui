...
export const downloadGeojson = (index: number) =>
    (dispatch: Dispatch, getState: () => any) => {
      const {project} = getState()
      const {currentProject, feeds, modifications} = project
      const variantName = currentProject.variants[index]
      ...
      modifications.filter(m => m.variants[index]).forEach(mod => {
        if (mod.type === 'add-trip-pattern') {
          const f = {
            type: 'Feature',
            properties: {
              name: mod.name,
          ...
          if (mod.segments) {
            ...
            mod.segments.forEach(seg =>
              f.geometry.coordinates.push(seg.geometry.coordinates[1]))
          }
          features.push(f)
        }
      })
      ...
      downloadJson({
        ...
        filename: `${description}-add-trip-alignments.geojson`.replace(/[^a-zA-Z0-9]/, '-')
      })
    }
