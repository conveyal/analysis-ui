...
export default function getFeedsRoutesAndStops({
  ...
  return async function (dispatch, getState) {
    ...
    if (shouldGetAllRoutesAndStops) {
      ...
      const newFeeds = get(json, 'bundle[0].feeds', []).map(labelAndSortFeed)
      dispatch(setFeeds(newFeeds))
      return newFeeds
    } else {
      ...
      if (unfetchedRouteIds.length > 0) {
        ...
        get(json, 'bundle[0].feeds', []).forEach((feed) => {
          const index = storeFeeds.findIndex((f) => f.id === feed.feed_id)
          if (index !== -1) {
            const storedFeed = {...storeFeeds[index]}
            storeFeeds[index] = {
              ...storedFeed,
              routes: unionBy(feed.routes, storedFeed.routes, 'route_id')
                .map(labelRoute)
                .sort(routeSorter)
            }
          } else {
            console.error(
              'Could not find feed in stored feeds',
              feed,
              storeFeeds
            )
          }
        })
        dispatch(setFeeds([...storeFeeds]))
      }
    }
