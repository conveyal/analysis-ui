...
export const deleteRegion = _id =>
  fetch({
    ...
    next: () => {
      ...
      return deleteLocally(_id)
    }
  })

export const load = id => async dispatch => {
  const region = await dispatch(fetch({url: `${REGION_URL}/${id}`}))
  ...
  await dispatch(loadBundles({regionId: id}))
  await dispatch(loadProjects({regionId: id}))
  await dispatch(loadProfileRequestSettings(id))
...
const checkLoadStatus = region => async dispatch => {
  ...
  const region = await dispatch(fetch({url: `${REGION_URL}/${region._id}`}))

  return dispatch(checkLoadStatus(response.value))
}
