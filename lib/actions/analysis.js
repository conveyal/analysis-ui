...
function serializeProfileRequest (opts) {
  ...
  let modifications = (opts.modifications || []).map(convertToR5Modification)
  let checksum = crc32(JSON.stringify(modifications))
    ...
    serverAction({
      ...
      next: async (response) => {
        if (response.status === 200) {
          ...
          return await timeout(fetchIsochrone(opts), 2000)
        }
      },
