...
class ScenarioEditor extends Component {
  ...
  render () {
    const {bundleId, data, deleteModification, id, mapState, modifications, variants, name, replaceModification, updateVariants, sessionToken, setMapState, user, project} = this.props
    ...
    return (
      <div>
        ...
        <Dock
          ...
          {sessionToken && user
            ? <div>Logged in as {user.email} <button onClick={this.logout}>Log out</button></div>
          ...
          <div>
            Project ID:<br/>
            {id ? name : <i>none selected</i>}<br/>
            <button onClick={this.openProjectSelectDialog}>Open Project</button>
          </div>

          <div>
            Bundle ID:<br/>
            {bundle ? bundle.name : <i>none selected</i>}<br/>
  ...
  renderProjectSelectModal () {
    ...
    if (projectSelectOpen) {
      return (
        <Modal
          isOpen
          onRequestClose={this.closeProjectSelectDialog}
          >
          <ul>{projectsList}</ul>
      ...
      return (
        <Modal isOpen onRequestClose={this.closeBundleUpload}>
          <UploadBundle />
        </Modal>
      )
