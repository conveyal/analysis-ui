...
const mapDispatchToProps = Object.assign({}, actionCreators, {copyFromScenario, getDataForModifications, push})
...
class ScenarioEditor extends Component {
  ...
  copyModificationsFromScenario = () => {
    this.props.copyFromScenario({
      ...
      toScenarioId: this.props.currentProject.id,
      variants: this.props.variants
    })
  }

  render () {
    const {currentBundle, currentProject, feeds, login, logout, outstandingRequests, push, userIsLoggedIn} = this.props
    const bundleIsLoaded = !!currentBundle
    const feedsAreLoaded = feeds && feeds.length > 0
    const projectIsLoaded = !!currentProject
    const allLoaded = feedsAreLoaded && projectIsLoaded && bundleIsLoaded
