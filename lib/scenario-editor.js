...
class ScenarioEditor extends Component {
  ...
  renderScenario () {
    const {bundleId, createVariant, data, deleteModification, id, mapState, modifications, variants, name, setActiveModification, setActiveTrips, setMapState, updateVariant} = this.props
    if (id && bundleId) {
      return <Scenario
        projectName={name}
        bundleId={bundleId}
  ...
  renderOutstandingRequests () {
    const { outstandingRequests } = this.props
