...
class ScenarioEditor extends Component {
  ...
  renderScenario () {
    const {bundleId, createVariant, data, deleteModification, id, mapState, modifications, variants, name, setActiveModification, setMapState, updateVariant} = this.props
    if (id && bundleId) {
      return <Scenario
        projectName={name}
        bundleId={bundleId}
