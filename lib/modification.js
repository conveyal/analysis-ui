    ...
    return (
      <div className='panel-heading'>
        <div className='panel-title clearfix'>
          <button className={`btn btn-${showingOnMap}`} onClick={this.toggleMapDisplay} title='Toggle map display'><i className='fa fa-globe'></i></button>
          <strong> {this.props.modification.name}</strong>

          <button className='btn btn-danger pull-right' onClick={this.delete} title='Remove modification'><i className='fa fa-close'></i></button>
          <button className='btn btn-default pull-right' onClick={this.toggleExpand} title={`${showOrHide} modification`}><i className={`fa fa-${iconName}`}></i></button>
        </div>
      </div>
    )
  }
