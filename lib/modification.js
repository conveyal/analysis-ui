...
function mapStateToProps ({
  ...
  const modification = scenario.modificationsById[id]
  return {
...
function mapDispatchToProps (dispatch, {
  ...
  return {
    remove: () => dispatch(deleteModification(id)),
    replace: (opts) => dispatch(setAndRetrieveData(opts)),
    setActive: () => dispatch(setActive(id)),
...
class Modification extends Component {
  ...
  renderExpanded () {
    const {allVariants, id, type, variants} = this.props
    const {feeds, feedsById, modification, mapState, setActiveTrips, setMapState} = this.props
    const props = {
      feeds,
      feedsById,
      mapState,
