...
export default class SelectPatterns extends Component {
  ...
  onMouseOverPattern (e) {
    ...
    let p = this.props.data.get(this.props.feed).routes.get(this.props.routes[0]).patterns.find(p => p.pattern_id === e.target.value)
  ...
  selectPatterns (e) {
    ...
    let trips = []
    let allPatterns = this.props.data.get(this.props.feed).routes.get(this.props.routes[0]).patterns
