...
export const reducers = {
  ...
  'set active modification' (state, action) {
    ...
    const isNotCurrentlyActive = !state.activeModification || !state.activeModification.id || state.activeModification.id !== modification.id
  ...
  'toggle expanded modification' (state, action) {
    const modification = {...action.payload}
    const expandedModificationId = state.expandedModificationId
    const modificationsById = {...state.modificationsById}
    const newModifaction = modificationsById[modification.id]
    let newState = {...state}
    newModifaction.expanded = !modification.expanded
    if (expandedModificationId && expandedModificationId !== modification.id) {
      const oldModification = modificationsById[expandedModificationId]
      oldModification.expanded = false
      modificationsById[oldModification.id] = oldModification
    }
    const modifications = Object.values(modificationsById)
    newState.modifications = modifications
    newState.expandedModificationId = modification.id
    return newState
  },
  'delete modification' (state, action) {
    ...
    let expandedModificationId = state.expandedModificationId
    if (expandedModificationId && expandedModificationId === modificationId) {
      expandedModificationId = null
    }
