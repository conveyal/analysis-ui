...
export const reducers = {
  ...
  'create variant' (state, action) {
    // there was a bug that caused variants to disappear and be set to null. This allows recovering from that bug.
    const existingVariants = state.variants ? state.variants : []
  ...
  'set modification' (state, action) {
    ...
    const modifications = [modification, ...state.modifications]
    const modificationsById = arrayToObj(modifications)
    return {
      ...state,
      modifications,
      modificationsById,
