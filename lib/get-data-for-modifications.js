...
export default function getDataForModifications ({ modifications, bundleId }) {
  ...
  for (let modification of modifications) {
    if (modification.type === 'remove-trips' || modification.type === 'remove-stops' || modification.type === 'adjust-speed' || modification.type === 'adjust-dwell-time' || modification.type === 'convert-to-frequency') {
  ...
  return authenticatedFetch(`/api/graphql?query=${encodeURIComponent(query)}&variables=${encodeURIComponent(JSON.stringify(vars))}`)
    .then(res => res.json())
    .then(res => {
