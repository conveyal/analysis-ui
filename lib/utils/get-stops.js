...
/**
 * when autogenerating stops between bona fide stops, how close can an autogenerated stop come to the next non-auto-generated stop, as a fraction of stop spacing? TODO Make configurable?
 */
...
export default function getStops (segments) {
  ...
  for (let segIdx = 0; segIdx < segments.length; segIdx++) {
    ...
    for (let i = 1; i < segment.geometry.coordinates.length; i++) {
      ...
      while (segment.spacing > 0 && distanceToLastStop + segment.spacing < distanceToLineSegmentStart + distanceThisLineSegment) {
        ...
        const pos = [c0[0] + (c1[0] - c0[0]) * frac, c0[1] + (c1[1] - c0[1]) * frac]
  ...
  return stops.filter((stop, stopIndex, stops) => {
    ...
    const spacedCorrectlyFromNextStop = nextStop.autoCreated || delta / spacingThisSegment >= MIN_SPACING_PERCENTAGE
