...
export default function convertConvertToFrequency (mod) {
  ...
  out.retainTripsOutsideFrequencyEntries =
    mod.retainTripsOutsideFrequencyEntries
  ...
  out.entries = mod.entries.map(
    ({
      ...
      if (exactTimes) {
        return {
          firstDepartures: getExactTimesFirstDepartures({
            headwaySecs,
            startTime,
            endTime
          }),
        ...
        const phasing =
          phaseAtStop != null
            ? {
              ...
              phaseFromStop,
              phaseFromTimetable:
                  phaseFromTimetable && phaseFromTimetable.length > 0
