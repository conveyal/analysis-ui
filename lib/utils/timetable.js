import format from 'date-fns/format'
import uuid from 'uuid'

export function create (segmentSpeeds = []) {
  const timetable = {
    id: uuid.v4(),
    startTime: 7 * 3600,
    endTime: 22 * 3600,
    dwellTime: 0,
    segmentSpeeds,
    headwaySecs: 600,
    exactTimes: false,

    // active every day
    monday: true,
    tuesday: true,
    wednesday: true,
    thursday: true,
    friday: true,
    saturday: true,
    sunday: true
  }

  return timetable
}

/** Get the first departures for a given timetable that is represented as exact times */
export function getExactTimesFirstDepartures ({ headwaySecs, startTime, endTime }) {
  const firstDepartures = [startTime]
  let departure = startTime
  // note that interval is [startTime, endTime), to match GTFS
  // https://groups.google.com/forum/#!topic/gtfs-changes/5u8yXBrpK2w
  while ((departure += headwaySecs) < endTime) firstDepartures.push(departure)
  return firstDepartures
}

const formatSecondsToHHMMSS = (s) => format(new Date(s * 1000), 'HH:mm:ss')

export function toString (timetable) {
  const startTime = formatSecondsToHHMMSS(timetable.startTime)
  const endTime = formatSecondsToHHMMSS(timetable.endTime)
  const headway = `${format(new Date(timetable.headwaySecs * 1000), 'mm')}m${format(new Date(timetable.headwaySecs * 1000), 'ss')}s`
  return `${timetable.name} ${startTime} to ${endTime} every ${headway}`
}
