...
export function create ({feedId, scenarioId, type, variants}) {
  const base = {
    expanded: false,
    id: uuid.v4(),
    name: toStartCase(type),
    scenario: scenarioId,
    showOnMap: true,
    type,
    variants
  ...
  if (isModificationWithSegments && modification.segments.length > 1) {
    ...
    for (let segIdx = 1; segIdx < modification.segments.length; segIdx++) {
      ...
      if (s0.stopAtEnd !== s1.stopAtStart) {
        debug(
          `End stop flag does not match start stop flag of next segment at ${segIdx -
            1}`
        )
        return false
      }
      ...
      if (s0.toStopId !== s1.fromStopId) {
        debug(
          `End stop ID does not match start stop ID of next segment at ${segIdx -
            1}`
        )
        return false
      }
      ...
      if (
        ...
        debug(
          `End coordinate does not match start coordinate of next segment at ${segIdx -
            1}`
        )
        return false
      }
