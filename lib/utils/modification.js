...
export function validateSegments(segments) {
  ...
  for (let segIdx = 1; segIdx < segments.length; segIdx++) {
    ...
    if (s0.stopAtEnd !== s1.stopAtStart) {
      return `End stop flag does not match start stop flag of next segment at ${segIdx -
        1}`
    }

    if (s0.toStopId !== s1.fromStopId) {
      return `End stop ID does not match start stop ID of next segment at ${segIdx -
        1}`
    }
    ...
    if (
      ...
      return `End coordinate does not match start coordinate of next segment at ${segIdx -
        1}`
    }
  }
}
