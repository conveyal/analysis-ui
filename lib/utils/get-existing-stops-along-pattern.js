...
export default function getExistingStopsAlongPattern(segments, stops) {
  const newSegments = []
  segments.forEach((segment) => {
    const line = turf.lineString(segment.geometry.coordinates)
    const segmentLengthKm = turfLength(line)
    ...
    stops.forEach((stop) => {
      ...
      if (
        stop.stop_id !== segment.fromStopId &&
        stop.stop_id !== segment.toStopId
      ) {
        ...
        if (pointToLineDistance(stopPoint, line) <= STOP_SEARCH_BUFFER_KM) {
          ...
          if (
            ...
            (!segment.stopAtEnd ||
              segmentLengthKm - pointOnLine.properties.location >
                APPROX_MIN_SPACING_KM)
          ) {
    ...
    const clusteredStops = clustersDbscan(
      turf.featureCollection(candidateStops),
      APPROX_MIN_SPACING_KM,
      {minPoints: 1}
    )
    ...
    clusteredStops.features.forEach((stop) => {
      ...
      if (
        !closestStopPerCluster[cluster] ||
        stop.properties.dist <
          closestStopPerCluster[cluster].properties.distance
      ) {
    ...
    if (closestStopPerCluster.length > 0) {
      closestStopPerCluster.sort(
        (a, b) => a.properties.location - b.properties.location
      )
