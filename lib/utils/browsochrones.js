...
async function newInstance () {
  if (!grids) throw new Error('Grids are not loaded!')
...
async function ensureScenarioLoaded ({ scenarioId, bundleId, modifications, workerVersion }) {
  ...
  if (scenarioHash !== instance.scenarioHash || workerVersion !== instance.workerVersion) {
    ...
    let request = getStaticSiteRequest({ modifications, bundleId, scenarioUniqueId, workerVersion })
...
async function getSpectrogramData ({ scenarioId, indicator }) {
  // NB no checks if this is the current scenario etc, assume call to ensureScenarioLoaded and ensureOriginLoaded
  let instance = instances.get(scenarioId)
...
export async function getIsochronesAndAccessibility ({ scenarioId, bundleId, modifications, isochroneCutoff, origin, indicator, workerVersion }) {
  await ensureScenarioLoaded({ scenarioId, modifications, bundleId, workerVersion })
  ...
  return {
    ...
    spectrogramData: await getSpectrogramData({ scenarioId, indicator }),
    indicator,
    isochroneCutoff
  }
}
