...
const WORKER_VERSION = 'v1.5.0-69-g58aa1eb'
...
async function retryFetch (url, opts, { timeoutMs = RETRY_TIMEOUT_MILLISECONDS, authenticated = false }) {
  let fetchFunc = authenticated ? authenticatedFetch : fetch

  let res = await fetchFunc(url, opts)

  if (res.status === 202) {
    debug(`202 response from server, retrying in ${timeoutMs / 1000}s`)
    await timeout(timeoutMs)
    return retryFetch(url, opts, { timeoutMs, authenticated })
  }
