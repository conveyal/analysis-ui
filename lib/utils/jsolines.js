  ...
  for (let origy = 0; origy < height - 1; origy++) {
    for (let origx = 0; origx < width - 1; origx++) {
      ...
      while (found[y * cWidth + x] !== 1) {
        ...
        if (!coord) {
          logError(
            `Unexpected coordinate shift from ${startx}, ${starty} to ${x}, ${y}, discarding ring`
          )
          break
        }
      ...
      if (found[y * cWidth + x] === 1) {
        logError(
          `Ring crosses other ring (or possibly self) at ${x}, ${y} coming from case ${idx}`
        )
        logError(
 ...
 */
function interpolate({cutoff, height, startx, starty, surface, width, x, y}) {
