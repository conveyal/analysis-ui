...
export default class TransitEditor extends FeatureGroup {
  ...
  getStopNear ({ lng, lat, radiusMeters, radiusPixels = 10 }) {
    ...
    if (zoom < MIN_STOP_SNAP_ZOOM) return null // don't snap at zoom levels where we're not showing stops
    ...
    const dLng = Math.abs(dLat / Math.cos(rad(lat)))
    ...
    const query = this.state.snapStops.filter((s) => s.stop_lat > minLat && s.stop_lat < maxLat && s.stop_lon > minLng && s.stop_lon < maxLng)
  ...
  toggleStop = (index) => {
    ...
    if (index < segments.length) {
      const newSeg = Object.assign({}, segments[index])
    ...
    if (index > 0) {
      const newSeg = Object.assign({}, segments[index - 1])
...
const ControlPoints = pure(({
  ...
  const lastSegmentCoordinates = lastSegment.geometry.type === 'LineString'
  ...
  return <g>
    ...
    {lastSegment && !lastSegment.stopAtEnd &&
      <StopMarker
        ...
        isStop={false}
        position={lastSegmentCoordinates}
        />
    }
  </g>
})
