...
import lonlng, {isEqual as coordinatesAreEqual} from 'lonlng'
...
class ControlPoints extends DeepEqualComponent {
  static propTypes = {
...
class Segments extends DeepEqualComponent {
  render () {
    const {onMousedown, segments} = this.props
    return <g>
      {segments
...
class Stops extends DeepEqualComponent {
  static propTypes = {
    onDelete: PropTypes.func.isRequired,
    onDragend: PropTypes.func.isRequired,
    onToggle: PropTypes.func.isRequired,
...
class StopMarker extends DeepEqualComponent {
  static propTypes = {
  ...
  render () {
    const {autoCreated, bearing, isStop, position, snapped} = this.props
    let icon
    if (isStop) {
...
class StopPopup extends DeepEqualComponent {
  render () {
    const {isStop, onDelete, onToggle} = this.props
    return <Popup>
      <div>
        <Button
          style='primary'
