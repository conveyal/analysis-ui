...
export default class SelectStops extends Component {
  static propTypes = {
    ...
    nullIsWildcard: PropTypes.boolean,
  ...
  getRouteStops (props) {
    ...
    if (!props.data.feeds.has(props.modification.feed)) {
      return []
    }

    let patterns = props.data.feeds.get(props.modification.feed).routes.get(props.modification.routes[0]).patterns
    ...
    let routeStops = [...routeStopIds].map((sid) => props.data.feeds.get(props.modification.feed).stops.get(sid))
  ...
  renderStops () {
    const feed = this.props.data.feeds.get(this.props.modification.feed)
    if (feed !== undefined) {
