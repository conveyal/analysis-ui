...
const _promptTextCreator = (label: string) => `Use "${label}" — WARNING: this will fail if this .jar does not exist on S3.`
...
export class SelectR5Version extends Component {
  ...
  _selectVersion = (result: ReactSelectResult) => {
    ...
    if (versionToNumber(result.value) < MIN_VERSION) {
      return window.alert(message('r5Version.invalidVersion', {
