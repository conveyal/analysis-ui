...
import {
  MINIMUM_R5_VERSION,
  RECOMMENDED_R5_VERSION
} from '../constants'
import * as select from '../selectors'
import type {ReactSelectResult} from '../../../types'
import {versionToNumber} from '../utils'
import type {UsedVersion} from '../types'
...
const _promptTextCreator = (label: string) => `Use custom version "${label}"`
...
export class SelectR5Version extends Component {
  ...
  _selectVersion = (result: ReactSelectResult) => {
    ...
    if (versionToNumber(result.value) < MIN_VERSION) {
      return window.alert(`${result.value} is not a valid version. Please select an option > ${MINIMUM_R5_VERSION}`)
    }
  ...
  render () {
    ...
    if (currentVersion !== RECOMMENDED_R5_VERSION) {
      options.push({
        ...
        label: versionToNumber(currentVersion) < MIN_VERSION
    ...
    usedVersions.forEach(uv => {
      if (uv.version === RECOMMENDED_R5_VERSION || versionToNumber(uv.version) < MIN_VERSION) return
      options.push({
        value: uv.version,
    ...
    return (
      <div>
        ...
        {versionToNumber(currentVersion) < versionToNumber(RECOMMENDED_R5_VERSION) &&
          <div className='alert alert-warning'>
            {message('r5Version.latestReleaseVersionNotSelected')}
          </div>}

        {usedVersions.length > 0 && usedVersions[0].version !== currentVersion &&
          <div className='alert alert-warning'>
