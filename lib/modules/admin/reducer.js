    ...
    case 'admin/SET_WORKERS': {
      ...
      action.payload.forEach(currentWorker => {
        ...
        const tasksPerMinute =
          Object.values(currentWorker.tasksPerMinuteByJobId).reduce(
            (a, b) => a + parseInt(b, 10),
            0
          ) || 0
        currentWorker.lastSeenAt =
          Date.now() - currentWorker.seenSecondsAgo * 1000
        if (index === -1) {
          workers.push({
            ...currentWorker,
