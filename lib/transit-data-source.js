  ...
  getDataForModifications ({ modifications, bundleId, forceCompleteUpdate = false }) {
    ...
    if (completeFetch) {
      ...
      let routesToFetch = [...routes].filter((r) => {
        for (let feed of this.data.feeds.values()) {
          if (feed.routes.has(r) && feed.routes.get(r).patterns != null) return false // no need to fetch this route
        }

        return true // we need to fetch this route, it's not in the data
      })
        ...
        .then((res) => {
          ...
          for (let feed of res.bundle[0].feeds) {
            for (let route of feed.routes) {
              ...
              this.data.feeds.get(feed.feed_id).routes.set(route.route_id, route)
            }
          }
