...
export async function getDataForModifications ({
  bundleId,
  ...
  if (completeFetch) {
    ...
    const routeIds = [...routes].filter((r) => {
      return !Object.values(fetchedData.data.feeds).includes((feed) => {
      ...
      if (json.bundle && json.bundle.length > 0) {
        for (let {feed_id, routes} of json.bundle[0].feeds) {
          for (let route of routes) {
            ...
            fetchedData.data.feeds[feed_id].routes.set(route.route_id, route)
          }
        }
