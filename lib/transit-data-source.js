...
class TransitDataSource {
  ...
  async getDataForModifications ({
    bundleId,
    forceCompleteUpdate = false,
    modifications
  }) {
    const completeFetch = forceCompleteUpdate || bundleId !== this.bundleId
    ...
    try {
      if (completeFetch) {
        ...
        const response = await authenticatedFetch(query.compose(query.data, vars))
        ...
        const response = await authenticatedFetch(query.compose(query.route, vars))
        const json = await parseJSON(response)
        ...
        if (json.bundle && json.bundle.length > 0) {
          for (let {feed_id, routes} of json.bundle[0].feeds) {
            for (let route of routes) {
              ...
              this.data.feeds[feed_id].routes.set(route.route_id, route)
            }
          }
