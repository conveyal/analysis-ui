...
export default class ScenarioMap extends Component {
  ...
  render () {
    ...
    return <LeafletMap
      ...
      {modificationsOnMap
        .filter((m) => m.type === 'add-stops' && (mapState.modification == null || mapState.modificationId !== m.id))
        .map((m) => <AddStopsLayer
          data={data}
      ...
      {modificationsOnMap
        ...
        .map((m) => <GeoJson
          ...
          // geojson layers don't update on props change, use key to ensure a new instance is created
          key={uuid.v4()}
          weight={3}
          />
        )}
  ...
  renderStateSpecificLayers () {
    ...
    if (mapState.state === 'stop-selection') {
      ...
      return <TransitEditor modification={m} replaceModification={replaceModification} data={data} extendFromEnd={mapState.extendFromEnd} allowExtend={mapState.allowExtend} />
