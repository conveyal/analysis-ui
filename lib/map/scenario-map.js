...
export default class ScenarioMap extends Component {
  ...
  render () {
    ...
    return <LeafletMap
      ...
      >
      <TileLayer
        url='http://{s}.tiles.mapbox.com/v3/conveyal.hml987j0/{z}/{x}/{y}.png'
      ...
      {modificationsOnMap
        .filter((m) => m.type === 'adjust-dwell-time' || m.type === 'convert-to-frequency')
        .map((m) => <PatternLayer
      ...
      {modificationsOnMap
        .filter((m) => m.type === 'remove-stops' || m.type === 'add-stops')
        .map((m) => <PatternLayer
  ...
  renderStateSpecificLayers () {
    const {data, mapState} = this.props
    if (mapState.state === 'stop-selection') {
      return <StopSelectPolygon {...mapState} />
    } else if (mapState.state === 'hop-selection') {
      return <HopSelectPolygon data={data} {...mapState} />
      ...
      const allowExtendFromEnd = mapState.type === 'add-trip-pattern' || mapState.modification.toStop === null
      const allowExtendFromStart = mapState.type === 'add-trip-pattern' || mapState.modification.fromStop === null
