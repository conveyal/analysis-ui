...
function mapStateToProps (state) {
  const {currentBundle, currentProject, feeds} = state.scenario
  return {
    ...
    projectIsReady: !!(currentBundle && currentProject && feeds && feeds.length > 0)
  }
}
...
class ScenarioMap extends Component {
  ...
  renderModifications () {
    ...
    // default location: Washington, DC because that's where @mattwigway lives
    const modificationsOnMap = modifications.filter((m) => m.showOnMap && m.feed)
    ...
    return <FeatureGroup>
      ...
      {modificationsOnMap
        .filter((m) => m.type === modType.ADD_STOPS && (mapState.modificationId == null || mapState.modificationId !== m.id))
        .map((m) => <AddStopsLayer
          feed={feedsById[m.feed]}
  ...
  renderStateSpecificLayers () {
    const {feedsById, mapState, modificationsById, replaceModification, setMapState} = this.props
    switch (mapState.state) {
      ...
      case mapStateType.SINGLE_STOP_SELECTION:
        ...
        return <StopLayer
          allowSelect
          key={modification.id + '_select'}
          modification={modification}
          nullIsWildcard
          onSelect={(stop) => {
            replaceModification(updateAddStopsTerminus({modification, which: mapState.which, newStop: stop, feed: feedsById[modification.feed]}))
            setMapState({})
          }}
