  ...
  render () {
    ...
    return <LeafletMap center={center} zoom={12}>
      ...
      {(this.props.modifications ? [...this.props.modifications.values()] : [])
        ...
        .map((m) => <AddStopsLayer key={`add-stops-layer-${m.id}`} data={this.props.data} modification={m} />)}
      ...
      {(() => {
        if (this.props.mapState.state === 'stop-selection') {
        ...
        } else if (this.props.mapState.state === 'add-trip-pattern' || this.props.mapState.state === 'add-stops') {
          ...
          let allowExtendFromEnd = this.props.mapState.type === 'add-trip-pattern' || this.props.mapState.modification.toStop === null
          let allowExtendFromStart = this.props.mapState.type === 'add-trip-pattern' || this.props.mapState.modification.fromStop === null

          let instance = new LeafletTransitEditor({ stops, stopIds, geometry, controlPoints, snapStops, allowExtendFromStart, allowExtendFromEnd })
