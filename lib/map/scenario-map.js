...
import {ADD_STOPS, ADD_TRIP_PATTERN, ADJUST_DWELL_TIME, ADJUST_SPEED, CONVERT_TO_FREQUENCY, REMOVE_STOPS, REMOVE_TRIPS} from '../utils/modification'
...
class ScenarioMap extends Component {
  ...
  renderModifications () {
    ...
    return <FeatureGroup>
      ...
      {modificationsOnMap
        .filter((m) => m.type === REMOVE_STOPS || m.type === ADD_STOPS || m.type === ADJUST_DWELL_TIME)
        .map((m) => <PatternLayer
      ...
      {modificationsOnMap
        .filter((m) => m.type === ADJUST_DWELL_TIME)
        .map((m) => <StopLayer
          dim={dim(m.id)}
          feed={feedsById[m.feed]}
          key={m.id}
      ...
      {modificationsOnMap
        .filter((m) => m.type === ADD_STOPS && (mapState.modificationId == null || mapState.modificationId !== m.id))
        .map((m) => <AddStopsLayer
          feed={feedsById[m.feed]}
