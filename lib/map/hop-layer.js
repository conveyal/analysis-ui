    ...
    for (let pattern of patterns) {
      ...
      let makeGeometryIfNecessary = () => {
        if (startHopIndex >= 0) {
          ...
          let fromStop = this.props.data.feeds.get(this.props.modification.feed).stops.get(fromStopId)
          // get feature at end of hop
          let toStopId = pattern.stops[prevHopIndex + 1].stop_id
          let toStop = this.props.data.feeds.get(this.props.modification.feed).stops.get(toStopId)
