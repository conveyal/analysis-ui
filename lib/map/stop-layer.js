...
export default class StopLayer extends Path {
  ...
  render () {
    ...
    if (!this.props.data.feeds.has(this.props.modification.feed) || this.props.modification.routes == null) return ret

    let patterns = this.props.data.feeds.get(this.props.modification.feed).routes.get(this.props.modification.routes[0]).patterns
    ...
    routeStops
      .map((stop) => circleMarker(latLng(stop.stop_lat, stop.stop_lon), {
        color: this.props.nullIsWildcard && this.props.modification.stops == null || this.props.modification.stops != null && this.props.modification.stops.indexOf(stop.stop_id) > -1 ? 
          this.props.selectedColor : this.props.unselectedColor,
        radius: 4
      }))
      .forEach((marker) => this.leafletElement.addLayer(marker))
