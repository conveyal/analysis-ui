...
export default class StopSelectPolygon extends MapControl {
  ...
  doSelect (polygon) {
    ...
    let hops = candidateHops.filter((hop) => {
      let s0 = this.props.data.feeds.get(this.props.modification.feed).stops.get(hop[0])
      let s1 = this.props.data.feeds.get(this.props.modification.feed).stops.get(hop[1])
