...
export default class PatternLayer extends MapComponent {
  ...
  getPatterns () {
    const {activePatterns, data, dim, modification} = this.props
    ...
    return filterPatterns({ activePatterns, dim, modification, patterns })
  }
...
function filterPatterns ({
  ...
  if (!dim && activePatterns) {
    return patterns.filter((pat) => activePatterns.indexOf(pat.pattern_id) !== -1)
  }
...
function filterSelectedPatterns ({
  ...
  if (selectedPatterns.length > 0) {
    return patterns.filter((pat) => selectedPatterns.indexOf(pat.pattern_id) !== -1)
  } else {
    return patterns
  }
}
