...
export default class TransitEditor extends FeatureGroup {
  ...
  renderSnapStops () {
    return [<StopLayer map={this.props.map} stops={this.state.snapStops} minZoom={MIN_STOP_SNAP_ZOOM} key='snapStops' />]
  }
  ...
  toggleStop (index) {
    ...
    if (index < segments.length) {
      ...
      // if it's not a stop, can't be snapped
  ...
  createMarker ({ index, coord, isStop, autoCreated, snapped }) {
    ...
    return <Marker
      ...
      draggable={!autoCreated} // TODO drag autocreated stops to fix them in place
      ...
      <Popup>
        <span>
          <a href='#' onClick={(e) => this.toggleStop(index)}>{isStop ? messages.transitEditor.makeControlPoint : messages.transitEditor.makeStop}</a>&span;
  ...
  getSegment ({ from, to, stopAtStart, stopAtEnd, fromStopId, toStopId, spacing = 400 }) {
