...
export default class TransitEditor extends FeatureGroup {
  ...
  createMarker ({ index, coord, isStop, autoCreated, snapped }) {
    ...
    let icon
    if (isStop) {
      let iconName = index === 0 && !autoCreated ? 'star' : 'subway' // first stop is a star
      if (autoCreated) icon = fontawesomeIcon({ icon: iconName, color: '#888' })
      else if (snapped) icon = fontawesomeIcon({ icon: iconName, color: '#48f' })
      else icon = fontawesomeIcon({ icon: iconName, color: '#000' })
    } else {
